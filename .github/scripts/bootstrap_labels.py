...
def create_label(headers, repo, name, color, description, dry_run):
    ...
    try:
        with request.urlopen(req, timeout=15) as resp:
            return (
                "created" if resp.status in (200, 201) else f"unexpected:{resp.status}"
            )
...
def main() -> int:
    ...
    for name, color, description in LABELS:
        status = create_label(
            headers, args.repo, name, color, description, args.dry_run
        )
        if status == "created":
            created.append(name)
            print(f"  + {name}")
            ...
            exists.append(name)
            print(f"  {'~' if args.dry_run else '='} {name}")
        else:
    ...
    if args.dry_run:
        ...
        print(
            f"Fertig: erstellt={len(created)}, vorhanden={len(exists)}, fehler={len(errors)}"
        )
