...
def gh_close_issue(repo: str, issue_number: int, token: str) -> None:
    url = f"{GITHUB_API}/repos/{ORG}/{repo}/issues/{issue_number}"
    httpx.patch(
        url, headers=_gh_headers(token), json={"state": "closed"}, timeout=15
    ).raise_for_status()
...
def gh_comment(repo: str, issue_number: int, body: str, token: str) -> None:
    ...
    httpx.post(
        url, headers=_gh_headers(token), json={"body": body}, timeout=15
    ).raise_for_status()
...
def _get_last_changelog_date(repo_path: Path) -> str | None:
    ...
    if not changelog.exists():
        return None
    m = re.search(
        r"##\s*\[.*?\]\s*[—-]\s*([0-9]{4}-[0-9]{2}-[0-9]{2})", changelog.read_text()
    )
...
def _call_llm(prompt: str, max_tokens: int = 600) -> str | None:
    ...
    if not api_key:
        print(
            "  OPENAI_API_KEY not set and ~/.secrets/openai_api_key missing — skipping LLM call"
        )
        return None
    try:
...
def _get_pyproject_extras(repo_path: Path) -> str:
    ...
    if m:
        parts.append(
            m.group(1).split("\n\n")[0]
        )  # stop before first blank line section
...
def _llm_summarize_changelog(
    repo_name: str, version: str, commits: list[str]
) -> str | None:
...
def update_changelog(
    repo_path: Path, version: str, *, use_llm: bool = False, repo_name: str = ""
) -> bool:
    changelog = repo_path / "CHANGELOG.md"
    ...
    llm_content = None
    if use_llm:
        llm_content = _llm_summarize_changelog(
            repo_name or repo_path.name, version, log_lines
        )
...
def run_update(
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        if not version:
            return {
                "success": False,
                "error": f"Cannot determine version — no pyproject.toml, VERSION file or __version__ found in {repo_name}",
            }

        subprocess.run(
            ["git", "config", "user.email", "cascade-agent@iil.pet"], cwd=repo_path
        )
        subprocess.run(
            ["git", "config", "user.name", "Cascade Agent [docu-update]"], cwd=repo_path
        )

        readme_changed = update_readme_version(repo_path, version)
        install_fixed = check_readme_install_command(repo_path)
        changelog_changed = update_changelog(
            repo_path, version, use_llm=use_llm, repo_name=repo_name
        )
...
def main() -> int:
    ...
    token = (
        get_secret("github_token", "PROJECT_PAT")
        or get_secret("github_token", "GITHUB_TOKEN")
        or ""
    )
    if not token:
        print(
            "ERROR: No GitHub token found — set PROJECT_PAT env var or create ~/.secrets/github_token",
            file=sys.stderr,
        )
        return 1

    platform_repo = os.environ.get("GITHUB_REPOSITORY", f"{ORG}/platform").split("/")[
        -1
    ]
    ...
    if "docu-update" not in labels:
        print(
            f"SKIP: Issue #{args.issue_number} hat kein 'docu-update' Label (Labels: {labels})"
        )
        return 0
    ...
    if not repo_name:
        print(
            f"ERROR: Cannot parse repo_name from issue title: {title!r}",
            file=sys.stderr,
        )
        return 1
    ...
    if result.get("skipped"):
        print(
            f"SKIP: {repo_name} — nichts zu aktualisieren (bereits auf v{result['version']})"
        )
        gh_comment(
            platform_repo,
            args.issue_number,
