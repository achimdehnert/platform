...
1. **Die neun Gate-Vorschlaege aus [#2234](https://github.com/achimdehnert/platform/issues/2234) sind jetzt elf — die zwei widerlegten Verzichte sind widerrufen** (Owner-Entscheid 2026-08-23). `gate-matches-spelling-not-substance` und `partial-fix-not-generalized-to-sibling-artifacts` standen seit demselben Tag in der `declined`-Liste und wurden binnen Stunden mechanisch widerlegt; sie sind aus `declined` **entfernt** (ein Zusatzfeld haette nicht gewirkt — `gate_deckung.py` und `retro_kpis.py` lesen die Liste ueber den Slug) und stehen mit ihrer Widerlegung in der neuen Registry-Sektion `widerrufen`. Gemessen: ungedeckte Slugs <!--kz:gate-pflicht-offen-->11, Deckung 26 %. Fuer den ersten liegt ein Nachfolge-Vorschlag mit Messpunkt vor: **loest der registrierte Drill den Fall aus, der im Slug-Namen steht?** **Daneben: <!--kz:gate-rueckfaellig-->1 von <!--kz:gate-gesamt-->27 Gates rueckfaellig** (`claim-before-cheapest-check`, Kalibrierfenster bis 2026-09-20, Stand 0 von 10). **Achtung bei der 1:** 11 der 26 Gates stehen auf `zu-frueh`, sechs davon seit dem 23.08. — kein Rueckfall ist dort **kein** Wirksamkeits-Beleg.
2. **[#2148](https://github.com/achimdehnert/platform/issues/2148): die drei lautlosen Wege — (a) und (c) behoben, (b) sichtbar gemacht.** (a) DOPPELLAUF misst laufende Container statt Manifeste; die sechs verwaisten Manifeste auf prod sind abgeloest (Befunde 9 → 4). (c) `cancelled` ist eine eigene Klasse im Deploy-Scan, und die Concurrency-Gruppe ist in risk-hub je Ziel-Umgebung getrennt ([risk-hub#668](https://github.com/iilgmbh/risk-hub/pull/668), gemergt) — **13 gleichlautende PRs stehen in der Flotte offen** ([#2229](https://github.com/achimdehnert/platform/issues/2229)). (b) Der `staging`-Default ist **gewollt** (org-Standard, kein Auto-Prod, steht so im Workflow); was fehlte, war das Signal — jetzt Phase `0.7.12 prod-wirkung`. **risk-hub steht 41 Commits hinter Prod** und braucht Dispatch + Freigabe-Klick.
...
5. **[#2086](https://github.com/achimdehnert/platform/issues/2086): acht Prod-Volumes ohne jeden Backup-Nachweis (2,1 GB).** Darunter `lora_media` (1229 MB, haengt an keinem Container), `bfagent_*`, `ttz_hub_pgdata`, `deployment_postgres_data`. Ursache: das Offsite-Skript sichert pgdumps aus **laufenden** Containern und Volumes nur nach dem Muster `minio|media|upload|documents` — ein verwaistes Volume faellt durch **beide** Netze.
6. **[#2087](https://github.com/achimdehnert/platform/issues/2087): shared-ci-Versionsbaender, Rest-Sweep.** Das v1.1.0-Band (15 Bibliotheks-Repos) und die v1.1.7-fw-Gates sind auf **v1.1.11**. Offen: App-Repos (v1.1.10 ×17, v1.0.11 ×2, wedding-hub mischt zwei Baender in einer Datei). Nicht gemessen: `ttz-lif`, `meiki-lra`.
7. **[#2088](https://github.com/achimdehnert/platform/issues/2088): 19 von 24 ADR-Uebergaben haben nie eine Antwort bekommen.** Bei einer (jetzt KONZ-047) lag eine entscheidungsreife Architekturfrage zehn Monate ausserhalb jedes Repos und waere mit der 30-Tage-Frist verfallen. Zwei Fragen vor der Welle: braucht die Klasse einen **Rueckkanal-Zwang** statt Verfall, und sollte die Schleuse beim Faelligwerden pruefen, ob der referenzierte ADR ueberhaupt **existiert**?
8. **[#2054](https://github.com/achimdehnert/platform/issues/2054): `hygiene_melder.py` meldet jede korrekt verteilte Hook-Kopie als Drift.** Er hasht den `MANAGED-BY`-Footer mit, den `hook-dist-drift.sh` ausblendet — Anzeige **invertiert**. `0.7.4 prio-referenzen` und `0.7.6 leseflaeche` haben dieselbe Wurzel: sie melden „Diskrepanz", wo ein Text einen gemergten PR als **historischen Kontext zitiert**. Ein Fix nur am Footer laesst die beiden anderen stehen.
9. **[#2119](https://github.com/achimdehnert/platform/issues/2119): Public→Private Welle 1 (22 Repos) — Klassifikation, Messung und F-Anleitung liegen vor, die Owner-Freigabe fehlt.** F (iilgmbh-REC-1: zweiter Org-Owner) ist die 10-Minuten-Owner-Aktion, die zugleich ADR-255 entsperrt.
10. **14 Draft-PRs aus dem LLM-Readiness-Audit warten auf Review/Merge** (alle CI-gruen ausser meiki-dms#15, dessen Lint-Rot vorbestehend und in [meiki-dms#16](https://github.com/meiki-lra/meiki-dms/issues/16) getrackt ist). Die beiden KONZ-Entwuerfe in [#2110](https://github.com/achimdehnert/platform/pull/2110) enthalten explizite Owner-Fragen.
11. **[#2114](https://github.com/achimdehnert/platform/issues/2114): infra-deploy-Workflows (inkl. Database Backup) laufen seit 2026-07-30 ins Leere** — self-hosted-Label `prod-server` nimmt keiner an, 24h-Queue-Timeout, still. Beruehrt Prio 5 (#2086).
12. **Gegenprobe zum Wochenlauf, unveraendert offen:** beim naechsten Lauf, der **ttz-hub** wirklich aendert, muessen Checks am erzeugten PR erscheinen. Der historische Beleg (0 Checks an ttz-hub#28) ist abgeschlossene Vergangenheit; offen ist nur die kuenftige Gegenprobe.
