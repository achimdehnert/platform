...
1. ADR-285 **Acceptance-PR** [#1416](https://github.com/achimdehnert/platform/issues/1416) — §9-Kriterien **1–3 sind grün**, der Pilot ist abgeschlossen. Der zuletzt offene CLI-Pfad ist gemessen: `/issues-offen platform-TESTMARKER-4711` wurde am 25.07. 12:13 UTC **getippt**, der Body kam an beiden `$ARGUMENTS`-Stellen substituiert zurück (Werkzeug 2.1.220), Footer aus der Skills-Lane; Artefakt [#1441](https://github.com/achimdehnert/platform/pull/1441) MERGED. Offen bleiben Kriterium 4 (ADR-229 → `superseded`, ADR-230-Amendment, `claude-skills.md`) und Kriterium 5 (D5-Rückbau-Prozedur) — beides zusammen ist der Acceptance-PR, der ADR-285 auf `accepted` hebt.
2. **Owner-Entscheid** [#1414](https://github.com/achimdehnert/platform/issues/1414) — `pytest tools/tests/ + CI-tote Testorte` **required** schalten? Technische Vorbedingung ist seit [#1443](https://github.com/achimdehnert/platform/pull/1443) (MERGED) erfüllt: der `paths`-Filter auf `pull_request` ist weg, der Job läuft immer und entscheidet intern über die teuren Steps (Zwilling-Muster) — ohne das hätte ein required Check jeden docs-only-PR dauerhaft auf `pending` blockiert. **Vor der Ruleset-Schaltung fehlt eine Gegenprobe:** ein docs-only-PR gegen die neue `main`, auf dem der Check-Name grün meldet statt zu hängen (auf [#1441](https://github.com/achimdehnert/platform/pull/1441) erschien er noch gar nicht — das ist das Vorher-Bild). Ruleset `main-required-checks` verlangt heute nur `guardian` + `gitleaks secret scan` (gegen die API geprüft). Branch-Protection-Änderung → nicht autonom.
