...
4. **Kalibrierung des Artefakt-Budget-Melders — Weg (a) gebaut 2026-08-16, jetzt läuft das Sammeln.** Der Melder protokolliert seitdem jedes Feuern (`gate_hits.notiere`, Slug `artefakt-budget-schwelle-erreicht`, in der Gate-Registry und drill-frisch) — und zwar nicht nur die PR-Zahl, sondern die drei Kandidat-Kriterien aus dem [#1640-Register](https://github.com/achimdehnert/platform/issues/1640#issuecomment-5302130572): `prs_seit_owner` · `repos` · `prod`. **Offen bleibt die Auswertung**, und die braucht Sitzungen: `python3 tools/claude-hooks/gate_hits.py --bericht --seit 2026-08-16` lesen, bevor über eine neue Schwelle entschieden wird. Die Schwelle selbst ist bewusst unverändert (absolute PR-Zahl) — welches Kriterium sie künftig trägt, ist die noch offene Owner-Entscheidung.
