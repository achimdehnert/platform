...
- **✅ Rotation #2 ausgeführt (2026-08-02 13:36 UTC) und nachverifiziert (19:10 UTC):** SHA256-Vergleich (nur Hash-Präfixe, nie Klartext) — `~/.secrets/orchestrator_mcp_api_key` = `/opt/mcp-hub/.env.prod` = Container-Env `mcp_hub_orchestrator_http`; Container-StartedAt 13:36:21Z (Neuerstellung, nicht bloß restart); der Vorgänger-Key (`.bak-rotation2`) ist nirgends mehr aktiv. **Ausführende Session unbekannt — es existierte kein Log-Eintrag** (Artefakt-Pflicht verletzt; dieser Eintrag holt die Dokumentation nach). Rest offen: echter Funktionsbeweis = Orchestrator-MCP-Connect aus einer ladenden Session (z. B. dev-hub). Rotationsskript aus `~/shared/` entfernt (Übergabeschleuse).
