...
1. **Wochenlauf-Beweis `Gen project-facts.md` (Montag 04:00 UTC) — der einzige offene Abnahmepunkt aus dem geschlossenen Melder-Vorgang.** Der Fix gegen die CI-Unterdrueckungsmarke ist auf `main`; dass dadurch bei neu erzeugten PRs der Required Check `ci / gate` wirklich erscheint, ist bisher **begruendete Erwartung, kein Messwert**. Am Montag pruefen: erzeugt der Lauf PRs, die Checks tragen und ohne Bypass mergebar sind? Falls nein, ist die Ursache eine andere als vermutet.
