...
1. Stub-Issues via Sonnet-Session (`/model sonnet` + `/issues-offen`)
2. Nachlauf [#1438](https://github.com/achimdehnert/platform/issues/1438) — stale ADR-Evidenzpfade nach dem `_ci-python`-Retire (ADR-174-Frontmatter + KONZ-021 `source_path` behaupten Gegenwart, die Beispiel-Snippets in ADR-021/022/120 dürfen historisch bleiben), veraltete Banner-Kommentare in 3 Fremd-Repos, 2 tote `validate-workflows`-Jobs
