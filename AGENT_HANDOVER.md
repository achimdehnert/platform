...
## ⚡ Aktueller Stand (2026-07-23 — ADR-280 §8.1 auf 6/6 komplettiert → ADR `accepted`; Print-Agent-Datenschutz vollständig (#1381 gemergt), Start-Hook-Mirror-Fix gemergt (#1382))
...
1. **Cloud-Reichweiten-Lücke [#1298](https://github.com/achimdehnert/platform/issues/1298):** Richtungsempfehlung liegt seit 2026-07-23 als Kommentar am Issue (Option 1 „repo-lokale `.claude/skills/`" vor Option 2 „Account-Skills"). Der im Issue genannte billigste Check ist **lokal nicht durchführbar** — die vier Cloud-Routinen sind keine Repo-Dateien, sie leben cloud-seitig. Nächster Schritt ist ein Listing der tatsächlichen Routinen (`/schedule` bzw. `CronList`) + Prüfung, ob eine davon einen `/`-Skill referenziert. **Bis dahin nichts bauen** — beide Optionen führen eine neue Verteil-Route ein und sind owner-/ADR-gegatet (ADR-280 §10 Option F).
2. **[#1378](https://github.com/achimdehnert/platform/issues/1378) bleibt nach dem Merge von #1382 noch offen:** die live laufende Kopie `~/.claude/hooks/managed/handover_prio_mirror.sh` ist generiert (`do_not_edit`) — der Fix wirkt erst nach Regeneration der `hooks`-Lane. Das ist ein gegateter Eingriff in die Maschinen-Installation (analog zum `skills`-Rollout heute früh), also **Owner-Entscheid**.
3. **[#1297](https://github.com/achimdehnert/platform/issues/1297) prüfen und schließen:** die beiden inhaltlich schließenden PRs sind gemergt ([#1377](https://github.com/achimdehnert/platform/pull/1377) Enrichment→Ollama, [#1381](https://github.com/achimdehnert/platform/pull/1381) Abfluss-Hinweis + `--profile iil-extern`). Kein Closing-Keyword im PR-Text (bewusst), daher noch offen — vor dem Schließen die Akzeptanzkriterien des Issues einmal gegenprüfen (Regel „Closes #N nur bei allen Kriterien").
