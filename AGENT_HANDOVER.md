...
1. **Cloud-Reichweiten-Lücke [#1298](https://github.com/achimdehnert/platform/issues/1298) — billigster Check ERLEDIGT (2026-07-23), Owner-Entscheid offen:** `RemoteTrigger {action:list}` (via `/schedule`) geprüft — es sind **6** Routinen (nicht 4), und **0 von 6** referenzieren einen `/`-Skill: Wiederverwendung läuft entweder inline im Prompt oder über repo-committete Dateien (`tools/registry-canonical.py`, `scripts/repo-registry.yaml`), die der Cloud-Checkout liest. Der Befund ist damit **Dokumentation, kein Ausfall** — keine Routine hängt heute an einem Skill und scheitert. Ergebnis + Tabelle als [Issue-Kommentar](https://github.com/achimdehnert/platform/issues/1298#issuecomment-5058012668). Empfehlung: **Option 4 (dokumentieren, nichts bauen)**; latenter Einzelfall `morning-briefing` dupliziert `/briefing`-Logik. **Nichts gebaut** — beide Baustellen-Optionen sind owner-/ADR-gegatet (ADR-280 §10 Option F); offen bleibt allein der Owner-Entscheid „Option 4 dokumentieren + schließen" vs. offenlassen.
...
4. Ausführungstreue-Audit [#1167](https://github.com/achimdehnert/platform/issues/1167): Nebenfund Namenskollision KONZ-platform-001 weiterhin offen
