...
1. **ADR-Sync nach DevHub reparieren** [#1447](https://github.com/achimdehnert/platform/issues/1447) — `Sync ADRs to DevHub` ist **10 von 10 letzten Läufen rot** (ältester geprüfter 23.07.), Ursache aus dem Log: `401 Unauthorized` beim Lesen von `docs/adr` über die GitHub-API aus dem dev-hub-Container; Token abgelaufen. Maskiert durch einen `Webhook fallback`-Step, der `success` meldet — der Run bleibt rot, wirkt aber selbstheilend, ist nicht required und läuft nach dem Merge, wo niemand hinschaut. **Zuerst zu klären, bevor irgendwas rotiert wird:** ob der Fallback tatsächlich synchronisiert oder nur einen Trigger absetzt, der an derselben 401 scheitert — billigster Check ist der zuletzt importierte ADR-Stand in dev-hub gegen `platform main docs/adr/index.json` (ADR-285 steht seit 25.07. auf `accepted`; fehlt das dort, ist der Fallback wirkungslos). Danach Token-Rotation (`/refresh-github-token`) + Verifikationslauf. **Secrets-/Infra-Arbeit → Owner-Freigabe, eigene Session** (Owner-Entscheid 25.07.: „machen wir morgen").
2. Stub-Issues via Sonnet-Session (`/model sonnet` + `/issues-offen`)
3. Nachlauf [#1438](https://github.com/achimdehnert/platform/issues/1438) — stale ADR-Evidenzpfade nach dem `_ci-python`-Retire (ADR-174-Frontmatter + KONZ-021 `source_path` behaupten Gegenwart, die Beispiel-Snippets in ADR-021/022/120 dürfen historisch bleiben), veraltete Banner-Kommentare in 3 Fremd-Repos, 2 tote `validate-workflows`-Jobs
