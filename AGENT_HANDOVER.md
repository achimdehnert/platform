...
- **⛔ ZUERST — die MEiKI-Anzeige darf nicht raus, solange ein Beleg fehlt.** Der Entwurf liegt im hnu-Postfach (`Entwürfe`, an `schmidt@actago.de`, 9 Anlagen, **nicht gesendet**). Er führt als Feststellung: „Für die Dokumenten-Steuerung besteht ein Auftragsverarbeitungsvertrag LRA–OCOS; die Anzeige ist insoweit erfolgt." Das ist die Angabe des Projektleiters — im HNU-Schriftverkehr **nicht auffindbar**. Belegbar ist die Gegenrichtung: `00891_Beauftragung_Ocos Solutions GmbH.pdf` kam von `beschaffung@hnu.de` über eBANF, also **HNU → OCOS**. Dann wäre OCOS Unterauftragsverarbeiter der Hochschule und es zählte der AVV LRA–HNU (Rüggenmann, 2026-04-07), nicht ein eigener LRA-Vertrag. **Ein Blick in die LRA-Vertragsakte entscheidet, ob [meiki-hub#138](https://github.com/meiki-lra/meiki-hub/pull/138) so gemergt und die Mail so gesendet werden kann.**
...
2. **[#1549](https://github.com/achimdehnert/platform/issues/1549) schließen — die Arbeit ist getan, das Issue weiß es nicht** — `devhub_beat` läuft über IaC auf 512M ([dev-hub d9d7735](https://github.com/achimdehnert/dev-hub/commit/d9d7735), live 44,6 %), `iil_authentik_worker` ist healthy, und die vier Dauer-Roten waren ein **Healthcheck-Konstruktionsfehler**, kein Ausfall: `pidof` matcht den Exe-Basenamen `python3.12`, nie das setproctitle-`celery` ([dms-hub#58](https://github.com/achimdehnert/dms-hub/pull/58), [coach-hub#58](https://github.com/achimdehnert/coach-hub/pull/58), beide **MERGED 2026-08-02 20:23 UTC**). Der jüngste Kommentar am Issue stammt von **19:58 UTC** — er ist älter als beide Merges und kennt die Auflösung nicht. **Nächster Schritt: Prod einmal auf 0 unhealthy nachmessen und mit diesem Beleg schließen** — nicht ungeprüft schließen. Der „⚡ Aktueller Stand" führt den Punkt bereits als „vollständig aufgelöst"; genau diese Lücke zwischen Prosa und Ticket ist hier der offene Rest.
3. **[#1682](https://github.com/achimdehnert/platform/issues/1682) — Megatest-Restabbau** (Nachfolger von Alt-Punkt 1): Budget-Abbau je Repo Richtung 0 und ein `owner`-Feld in `repo-registry.yaml` für die 5 Fremd-Org-Repos (meiki-lra/iilgmbh/ttz-lif), die der `MEGATEST_READ_TOKEN` nicht erreicht; optionaler Token-Tausch. Der Megatest selbst ist grün (Main-Lauf [30758611903](https://github.com/achimdehnert/platform/actions/runs/30758611903), `success`) — offen ist nur der Abbau.
