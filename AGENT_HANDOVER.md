...
4. **mcp-hub-Deploy auf `ce79c2aa` verifizieren** — Run [30490611598](https://github.com/achimdehnert/mcp-hub/actions/runs/30490611598) stand beim Session-Ende noch auf **`queued`** (seit 20:59, self-hosted Runner-Warteschlange). Der Merge von [#186](https://github.com/achimdehnert/mcp-hub/pull/186) ist durch, der **Deploy nicht belegt** — „main grün" ist nicht „Prod aktuell". Erst nach grünem Deploy den scharfen Check fahren: ein `workflow_execute` gegen ein absichtlich ungültiges Modell muss `error: llm_call_failed` samt Modell und Provider liefern; vorher kam derselbe Fall als leerer Inhalt mit 0 Token zurück.
