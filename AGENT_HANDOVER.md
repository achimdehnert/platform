...
**Sitzung 8ab4ba5c (2026-09-07, Owner „35 Ziel: keine warnzeilen" → „36-42 go; 49 still legen; discord rueckbau" → „60 48 50 51-59 go" → „75 go reparieren" → „91/95/96 stoppen, 92/93/94/98 go" → „103/108/111/112/121 go" → „132 133 go"):** Zielzustand, Scope-Checkpoint und alle Owner-Gates in #2895. Runner `session_start_checks.sh platform` **16 → 8 WARN** (33 PASS); von den 8 loesen sich 3 selbst (eigene Sitzung, Reconcile-Nachtlauf, gemergter Docs-PR), 5 tragen einen Anker. **Werkzeug-Fixes gemergt** (alle in #2895 verlinkt)**:** Speicher-Melder (#2887), Prod-Wirkung (#2888), Deploy-Scan (#2892), prio-referenzen (#2890) + Nullstellung `geschaerft_am` (#2901), Reconcile C5 je Host (#2893), origin-tls Tunnel-Ingress (#2894), Flottenbild-Absturz an YAML-Datum — Timer war seit 01.09. blind (#2900), Schleuse 90-Tage-Regel (#2903), Melder-Register 9+18 Leser (#2889, #2905), Cron-Melder R2 (#2906), 6 Gates behandelt + 11 Slugs gedeckt (#2909), Docker-Praevention als IaC (#2918), Rotation Phase 1 (#2911, #2917), Ollama-Host/Groq (#2916), sevdesk-Ausgangsrechnung (#2926). **Deklarationen:** bahn-hub + onboarding-hub stillgelegt, discord-owner zurueckgebaut (#2899, #2902). **Deploys je auf Owner-Wort:** ausschreibungs-hub, tax-hub, risk-hub (nach litellm-OOM-Fix risk-hub#743), dev-hub — alle prod = main gemessen. **Flottenbild (Item 73):** ib_gateway, risk_hub-Altstack (7 Container), cadhub-api (5418 Restarts) gestoppt; `dev_hub_staging_worker` healthy nach Prozess-Healthcheck; Swap-Reset scheiterte am OOM-Killer. **Lease-Hygiene:** 130 → 53 Leases, 10 WIP-Draft-PRs, drei Reversions-Worktrees verworfen. **sevdesk:** Ausgangsrechnung 20260907-288 als Entwurf (650/123,50/773,50 €), Kunde neu. **Eigene Fehler:** doppelter `healthcheck`-Schluessel brach den dev-hub-Prod-Deploy (Rollback griff, Prod unversehrt, Fix dev-hub#341); ein wg0-Schluessel landete durch Terminal-Umbruch im Transkript und wurde sofort rotiert. **Lehren:** `secrets: inherit` wirkt nicht ueber die Org-Grenze (#2922); Compose nur mit `docker compose config` pruefen, nie `yaml.safe_load` allein; `pr_merge_sa.py` liest den Freigabe-Vermerk nur aus dem Issue-**Body**. **Offen (Owner):** #2922 Fleet-Scan · #2923 Staging-Deploy strukturell · #2929 beleg_entwurf.py ohne Tests · #2904 C4:2287 · wg0 Schritt 3+4 · Reboot mit Docker-Praevention · Rotation Phase 2 · PyPI-Konto achimdehnert · Rechnungsentwurf pruefen und senden.
