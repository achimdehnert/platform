...
2. **Waisen + Doppel-Nummern im ADR-Sync** [dev-hub#157](https://github.com/achimdehnert/dev-hub/issues/157) — `import_adrs` löscht nie: sechs Zeilen (`ADR-013/-014/-023/-033/-063/-2026`) haben keine Datei mehr in git. Getrennt davon tragen in `bfagent` (16 Dateien → 13 Zeilen) und `risk-hub` (34 → 33) mehrere Dateien dieselbe ADR-Nummer. Vorschlag im Ticket: `--prune` als **opt-in** (nie Standard, sonst löscht ein API-Fehlschlag flächig) und für die Doppelten erst messen, dann entscheiden.
