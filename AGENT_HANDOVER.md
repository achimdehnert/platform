...
>
> **Fortschritt 2026-07-06:** Prio 1 (Wave 3 Phase 1): **learn-hub#25 offen** (`name: "CI"`-Override entfernt → Checks jetzt `ci / …` lowercase, `ci / gate` folgt bei Suite-Ende; kein Merge). **recruiting-hub#10 + travel-beat#55 BLOCKIERT** — #811-Befund „KEIN PR-getriggertes ci.yml" ist veraltet: beide haben bereits bespoke PR-CI mit echten Tests; vorgeschriebener `skip_tests`-Fix würde Coverage verlieren → STOP + Optionen in platform#811-Kommentar 07-06 (Entscheid Achim nötig). Prio 2 (Deploy-Health, nie autonom): **billing-hub** Deploy rot = `deploy / 🐳 Build` GHCR-Push `403 Forbidden` (Token/packages-Perm; Image nicht gepusht); **cad-hub** Deploy rot = Image gepusht OK, nur GHA-Cache-Finalize `i/o timeout` (transient, Re-Run heilt). Beide auf docs-only-ADR-Push 07-04 (deploy-on-push ohne paths-Filter), **`deploy / 🚀 Production` skipped → prod-Images unberührt**.
