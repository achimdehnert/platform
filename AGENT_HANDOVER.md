...
2. **Review-Stau abgebaut — die 5 Reste sind ROT, nicht wartend:** nur noch [#892](https://github.com/achimdehnert/platform/pull/892)/[#893](https://github.com/achimdehnert/platform/pull/893)/[#986](https://github.com/achimdehnert/platform/pull/986)/[#1005](https://github.com/achimdehnert/platform/pull/1005)/[#1007](https://github.com/achimdehnert/platform/pull/1007) offen — **alle 5 `CONFLICTING`/`DIRTY`** (verifiziert 2026-07-21 via `gh pr view --json mergeable`; Achtung: liefert erst `UNKNOWN`, GitHub rechnet lazy — Re-Poll nötig). Sie brauchen **Rebase**, nicht Review. Ältester Stand 07-11.
3. Print-Agent ruft bei **jedem** PDF einen externen US-LLM ([#1297](https://github.com/achimdehnert/platform/issues/1297)) — Defaults `cerebras`/`groq` hart im Code (`print_agent.py:268/269`), kein Guard, kein Hinweis. Realer Abfluss am 07-21: Kundenanschrift ging an Groq. Lokales Ollama gemessen ausreichend (3B: 18 s, valides JSON). Kein PR, weil Richtungsentscheid (externe Anbieter künftig nur per Opt-in?).
4. KONZ-018 W1: testkit-Dedup, Freshness-Pilot promptfw
5. Stub-Issues via Sonnet-Session (`/model sonnet` + `/issues-offen`)
6. Gate für tote `implementation_evidence`-Pfade ([#1289](https://github.com/achimdehnert/platform/issues/1289)) — erst SUGGEST/non-gating, Baseline sichten vor Gating (sonst Alarm-Müdigkeit).

> **Erledigt 2026-07-21 (Vormittag, Aufräum- + Infra-Session):** **Prio 3 „lügender Index" abgeschlossen** ([#1288](https://github.com/achimdehnert/platform/pull/1288) GEMERGT) — ADR-158 `implementation_status` von `implemented` auf `partial` korrigiert (3 tote Evidence-Pfade unter `packages/docs-agent/`, seit 2026-04-23 in `_ARCHIVED/`; D-2-Generator ging nie in Betrieb, `docs/reference/*.md` sind Handpflege trotz `DO NOT EDIT`-Header), ADR-072-Rollout-Tabelle auf belegten Stand zurückgesetzt (widersprach der eigenen ADR-237-Frontmatter-Korrektur: tax-hub fehlte ganz, travel-beat stand auf „In Arbeit"). **Bewusst nicht nachgemessen** (wäre Scope-Eskalation in 4 App-Repos) → ungemessene Repos tragen jetzt explizit `❔ nicht nachgemessen` statt geschätztem Status. Gate-Lücke getrackt ([#1289](https://github.com/achimdehnert/platform/issues/1289)) · [#1013](https://github.com/achimdehnert/platform/pull/1013) + [#954](https://github.com/achimdehnert/platform/pull/954) gemergt · **Branch-Hygiene: 442 lokale Branches gelöscht (512 → 47)**, Restore-Manifest `~/shared/branch-cleanup-platform-2026-07-21.txt`. **Lehre (Branch-Cleanup):** der erste Durchlauf stufte 51 gemergte Branches falsch als „lokal voraus" ein — nicht wegen Divergenz, sondern weil der gemergte `headRefOid` lokal fehlte und der Ancestor-Check deshalb scheiterte. Nach `git fetch origin <oid>` waren alle 51 sauber entscheidbar; echte Divergenz lag bei 7, nicht 56. `git branch --merged` ist bei Squash-Merges unbrauchbar — Kriterium ist PR-State `MERGED` **und** Tip ⊆ `headRefOid`.
> **Erledigt 2026-07-21 (Infra-Entscheid GPU, kein Code):** Recherche + eigene Messung → **kein GPU-Server gemietet**. illustration-hub läuft auf der vorhandenen RTX 4090 (Flux + 32B parallel passt, vom Owner gemessen), meiki-Pilot + Dokumentenlast auf CPU des Dev-Servers. Gemessen 2026-07-21 auf 88.99.38.75 (16 Kerne, geteilte Last), Print-Agent-Prompt: `qwen2.5:3b` 7,3 tok/s / 18 s · `7b` 3,6 tok/s / 51 s · `14b` 1,1 tok/s / 153 s — alle valides JSON. Gegenrechnung: Hetzner GEX44 hat **20 GB** (weniger als die 4090; Primärquelle, keine Mindestlaufzeit, FSN1), VRAM-gleicher Scaleway L4 kostet 575 €/Mon netto bei ~⅓ Speicherbandbreite. **Nicht belegt geblieben:** GEX44-Monatspreis (Seite rendert clientseitig), Stromkosten, Preisknick Consumer↔Datacenter — 14 von 25 Recherche-Claims wurden adversarial verworfen. **Auflage für meiki:** gegen austauschbaren OpenAI-kompatiblen Endpunkt bauen, sonst wird der spätere Umzug ins LRA-RZ zum Umbau.
