...
**Diese Session (2026-07-10):** Ad-hoc-Mailversand an Auftraggeber → User-Anweisung „Mails von hier immer über Mittwald (ad@dehnert.team)" → Skill `/send-mail` gebaut ([#1039](https://github.com/achimdehnert/platform/pull/1039)), gehärtet ([#1050](https://github.com/achimdehnert/platform/pull/1050)), Policy nachgezogen ([#1051](https://github.com/achimdehnert/platform/pull/1051)), cc-skill-dist-Rollout (doctor 7→0), zwei adversariale Retros ([#1048](https://github.com/achimdehnert/platform/pull/1048) gemergt, [#1055](https://github.com/achimdehnert/platform/pull/1055) offen).
...
- **Incr-Retro (#1055, Review ausstehend, auto-merge armiert):** 6/7 SURVIVES, 1 REFUTED. Hoch: **Review-Gate 5b prüft lokal 388 vs. CI 486 Tests** — Makefile-Kommentar „identisch zu tools-tests.yml" ist falsch (4 CI-Testorte fehlen lokal). Hoch: Hook-Patch war untracked → in dieser session-ende ins `~/.claude`-Repo committet. Mittel: Guard-Falsch-Positiv (`| tail` + `.env`-Prosa) ohne Tracking-Anker.
- **OFFEN (Human/Zuruf):** (a) [#1055](https://github.com/achimdehnert/platform/pull/1055) reviewen (auto-merge armiert); (b) Live-Rollout `/send-mail` v1.1 nach `~/.claude/commands` (Kopie ist v1.0; cc-skill-dist gegated, User-Go); (c) Maßnahmen **I3–I7** aus #1055 §7: Makefile↔CI-Angleich, Hook-Doktrin/Grenzen, Rotation-Test, FP-Anker, Registry-Schwelle; (d) Memory-Kandidat `hooks-repo-commit-pflicht` (#1055 §6); (e) `platform-pinned` dirty → `~/.claude/policies` stale (Refresh beim Start geblockt); (f) Outline-`/knowledge-capture` bewusst ausgelassen (Wissen liegt in git-Retros + CC-Memories) — bei Bedarf nachziehen.
