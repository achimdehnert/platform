...
3. **[mcp-hub#187](https://github.com/achimdehnert/mcp-hub/pull/187) mergen und den Outline-MCP neu starten** — **bis dahin ist jeder Outline-Zugriff blockiert.** `knowledge.iil.pet` steht seit 2026-07-29 hinter Cloudflare Access (App `43e13b3b-…`, zwei Richtlinien: Owner-Adressen und `non_identity` für gültige Dienstzugänge). Der laufende MCP stammt von vor dem Patch, schickt die `CF-Access-*`-Kopfzeilen nicht und bekommt **302 statt JSON** — das ist bereits eingetreten, nicht theoretisch. Der PR enthält beides: den Client-Patch und den fehlenden Export im Startskript. Ein fertiger Lesson-Entwurf zum cgroup-Thema wartet solange unter `~/.claude/outline-pending/2026-07-29-cgroup-speicherdruck.md` und gehört nach dem Neustart nachgetragen. **Wichtig für jede weitere Access-Regel:** Das Zero-Trust-Konto hat genau **einen** Anbieter (GitHub), kein Einmal-PIN — Cloudflare liefert immer `admin@wir-digital.de`. Andere Adressen in einer Richtlinie sind wirkungslos; wer nur sie einträgt, sperrt sich aus. Der IdP-Endpunkt ist zudem **nur** mit `cloudflare_write_token` lesbar, `cloudflare_api_token` gibt dort einen Berechtigungsfehler, der wie eine leere Liste aussieht.
