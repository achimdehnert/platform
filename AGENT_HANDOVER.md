...
3. **[#1544](https://github.com/achimdehnert/platform/pull/1544) reviewen und mergen, danach die Konto-Config umstellen** — `MAIL_LOGIN` trennt Anmeldename und Absenderadresse. Bisher diente `MAIL_FROM` beidem: beim HNU-Postfach ist der Anmeldename ein bloßer Benutzername, also trug jeder Entwurf `dehnert` als Absender; wer stattdessen die Adresse eintrug, fand kein Zugangspaar mehr und kam gar nicht erst hinein. Alle vier Aufrufstellen sind umgestellt (`send_mail`, `draft_mail`, `read_mail.connect`, `organize_mail`) — eine allein hätte den Lesepfad gebrochen. **Nach dem Merge** in `~/.claude/mail-hnu.env` die beiden auskommentierten Zeilen setzen (`MAIL_LOGIN=dehnert`, `MAIL_FROM=Achim.Dehnert@hnu.de`); **vorher nicht** — der installierte Stand im Checkout käme damit nicht mehr ins Postfach.
4. **Verteiler-Drift schließen (DRIFT-SCORE 1)** — `doctor.py` meldet `read-mail.md` als `copy-stale`. Ursache ist bekannt und selbst verursacht: die Quelle wurde am 29.07. dreimal geändert (#1531, #1536, #1538). Solange die Kopie alt ist, kennt die nächste Session die neuen Werkzeuge nicht (`mail_view.py`, `mail_link_server.py`, `anker.py`, `port_freigeben.py`) und auch nicht die systemd-Warnung. Regeneration der Live-Lane ist eine Maschinen-Änderung und braucht Owner-Freigabe: `python3 tools/cc-skill-dist/generate.py --allow-live`.

> **Erledigt 2026-07-29 (Parallel-Session Mail-Links):** **Alt-Prio 3 (#1511 mergen) entfällt** — der PR ist **CLOSED, nicht gemergt** (per `gh pr view` geprüft, nicht aus dem Vor-Stand übernommen); der Session-Stand vom 29.07. steht bereits auf `main`. An seine Stelle rücken zwei Punkte dieser Session. Liste bleibt bei 4 Einträgen, lückenlos durchnummeriert (Maschinen-Vertrag: `claude-next-sync` matcht nur ganze Zahlen). **Neu gebaut und gemergt:** [#1531](https://github.com/achimdehnert/platform/pull/1531) `mail_view.py`, [#1535](https://github.com/achimdehnert/platform/pull/1535) `anker.py` + `/a/`-Route, [#1536](https://github.com/achimdehnert/platform/pull/1536) `port_freigeben.py`, [#1538](https://github.com/achimdehnert/platform/pull/1538) systemd-Falle. Der Dienst läuft als User-Unit `mail-links.service` (enabled, neustartfest verifiziert). **Nicht mit erledigt:** die Rolle `dsb` hat weiterhin keine Kontaktdaten ([#1481](https://github.com/achimdehnert/platform/issues/1481)) — bei `hnu` war derselbe Platzhalter drin und wäre an eine Studentin hinausgegangen; korrigiert.
>
