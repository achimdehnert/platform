...
1. **[#1650](https://github.com/achimdehnert/platform/issues/1650) — Gate-Audit: noch 6 gate-pflichtige Slugs ohne Entscheidung (Stand 2026-08-12).** Die Familie „aufgeschobene Arbeit ohne Anker" ist seit [#1897](https://github.com/achimdehnert/platform/pull/1897) durch das `aufschub-anker`-Gate gedeckt (4 Slugs, inkl. `workaround-without-tracking-anchor` + `tracking-doc-stale-after-new-occurrence`). Session 2026-08-12 (gate-registry-nachtrag): vier **existierende, aber unregistrierte** Gates registriert + gedrillt (`parallel-session-pr-collision`, `worktree-midsession-accumulation`, `lint-failure-no-local-gate`, `ci-gate-maskiert-failure`) und `retro_kpis.py` meldet jetzt selbst jeden GATE-PFLICHT-Slug ohne Registry-Eintrag (Abnahme-Kriterium 3 — der Audit-Befund reproduziert sich damit automatisch statt per Handarbeit). **Offen bleibt Kriterium 1:** für `always-instruction-without-enforcement`, `autonomous-no-human-review`, `ci-replace-requires-job-catalog-diff`, `issue-not-reconciled-after-cross-repo-fix`, `platform-pinned-perma-dirty-loop`, `untested-tool-module-green-gate` je Owner-Entscheidung Gate-bauen vs. bewusste Nicht-Gate-Einstufung — Entscheidungsvorlage als Kommentar im Issue.
