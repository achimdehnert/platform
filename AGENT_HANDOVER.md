...
3. **`devhub_beat` und die toten Health-Checks** ([#1549](https://github.com/achimdehnert/platform/issues/1549)) — `devhub_beat` bei 89,5 % seines 256-MiB-Limits (echter Verbrauch, kein Seiten-Cache, also nicht durch die Host-Entlastung geheilt) und Health-Checks, die seit Tagen rot sind: `iil_authentik_worker` seit **12 Tagen**, Log endet am 18.07. mit `code -9` — der Server antwortet weiter, der Worker ist tot. **Erledigt 2026-07-31, anders ausgegangen als erwartet:** das Gegenlesen des ersten scharfen Laufs ist durchgeführt — es gibt **keinen** täglichen 03:30-Ingest auf dem Prod-Host, auf keiner Schicht (`grep mail_agent` über 24 h Logs: 0 Treffer · Celery-Registry ohne `mail`-Task · `PeriodicTask`-Tabelle ohne `mail_agent` · Bind-Mount beidseitig nicht vorhanden). Damit entfällt dieser Lauf als Belastungstest für das 256-MiB-Limit; die Speicherfrage von `devhub_beat` bleibt davon unberührt offen. Volle Messung im Stand-Block oben (⚠ Richtigstellung 2026-07-31), getrackt als [dev-hub#187](https://github.com/achimdehnert/dev-hub/issues/187).
