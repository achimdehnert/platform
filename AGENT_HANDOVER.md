...
## ⚡ Aktueller Stand (2026-08-11 — FW-Speicher- und Drift-Welle abgeschlossen; wedding-hub-Stilllegung wartet auf Owner-Formel)
...
- **wedding-hub-Stilllegung** (dritter Owner-Auftrag): `/delete-repo` Phase 0–2 durch — Identität via API (ID 1154747767), Backup verifiziert (`~/backups/repo-graveyard/achimdehnert--wedding-hub--2026-08-11/`: mirror+bundle+fsck+SHA, Issues/PRs-Export), App-Container gestoppt (DB läuft für Dump), 5 PRs + 6 Issues mit Vermerk geschlossen. **Wartet:** wörtliche Phase-3-Formel („achimdehnert/wedding-hub" + ARCHIVIEREN), DB-Dump-Einzeiler (Classifier-gated), danach Archiv + Tombstone + `canonical.yaml`-Folge-PR.
...
- **Offen (Owner-Zug):** risk-Prod-Redispatch (uv.lock-Fix #578 gemergt; Prod läuft noch 0.11.5) · Review #1917 · 137-hub GHCR dauerhaft ([137-hub#86](https://github.com/achimdehnert/137-hub/issues/86); Host-PAT-Relogin ist Übergangslösung, Wurzel #1078) · travel-beat Prod-Migration rot ([travel-beat#79](https://github.com/achimdehnert/travel-beat/issues/79), Vorbestand: erster Deploy seit 12.07.) · tax-Preload kommt mit dem nächsten Prod-Trigger.
...
1. **[#1650](https://github.com/achimdehnert/platform/issues/1650) — Gate-Audit: 7 von 18 gate-pflichtigen Slugs haben kein Gate.** Stand gemessen am 2026-08-10 gegen `docs/governance/gate-registry.json` (7 Gates registriert, alle drill-frisch): ohne Gate sind noch `always-instruction-without-enforcement`, `ci-gate-maskiert-failure`, `ci-replace-requires-job-catalog-diff`, `issue-not-reconciled-after-cross-repo-fix`, `platform-pinned-perma-dirty-loop`, `tracking-doc-stale-after-new-occurrence`, `workaround-without-tracking-anchor`. Der achte aus dem Audit (`deferred-item-no-tracking-issue`) ist seit [#1645](https://github.com/achimdehnert/platform/pull/1645) gebaut. Empfehlung unverändert aus dem Issue: **nicht alle einzeln** — die zwei verbliebenen Slugs der Familie „aufgeschobene Arbeit ohne Anker" (`workaround-without-tracking-anchor`, `tracking-doc-stale-after-new-occurrence`) mit **einem** gemeinsamen Gate abdecken. Die Audit-Zahl ist eine **Obergrenze der Lücke** (Suche geht über den Slug-Namen) — billigster Gegencheck ist ein Blick pro Slug, ob die Wirkung anderswo schon erzwungen wird.
