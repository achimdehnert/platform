...
4. **Kalibrierung des Artefakt-Budget-Melders — nächste Sitzung messen (Owner-Entscheidung 2026-08-15).** Bilanz des 15.08.: er feuerte **achtmal** (bei 4/5/6/7/8/9/10/11 PRs) und bekam achtmal dieselbe Antwort — im Auftrag, jede Eskalation einzeln owner-freigegeben. **Null Treffer.** Seine Schwelle ist eine absolute PR-Zahl (`ARTEFAKT_BUDGET_PRS`, Default 4), seine Frage aber: stammt das aus einem Auftrag oder aus einer ungefragt gewachsenen Kette? Genau das misst er nicht. Kandidat-Kriterien im [#1640-Register](https://github.com/achimdehnert/platform/issues/1640#issuecomment-5302130572): PRs **ohne** vorangehende Owner-Nachricht seit dem letzten Checkpoint · Anzahl berührter **Repos** · Erreichen eines Prod-/Publish-Schritts.
> **⚠ Wer das misst, stolpert sonst über dieselbe Falle wie diese Sitzung:** `tools/claude-hooks/artefakt_budget.py` schreibt **nirgends auf Platte** — kein `gate_hits.notiere()`, kein State ausser einer tmp-Zaehlerdatei je Session. Die acht Auslösungen des 15.08. haben **keine Spur** hinterlassen; die Zahl oben stammt aus dem Sitzungsverlauf, nicht aus einem Protokoll. „Über mehrere Sitzungen messen" geht damit **nicht** von allein. Zwei Wege, beide klein: (a) den Melder analog zu den Welle-1-Scannern an `gate_hits.notiere()` hängen (~5 Zeilen, Muster in `deferred_item_scanner.py` Z. 115) und **dann** ein paar Sitzungen laufen lassen; (b) je Sitzung von Hand mitschreiben, wie oft er feuerte und wie oft die Antwort „ja, im Auftrag" lautete. **(a) zuerst** — sonst misst man wieder Erinnerung statt Daten, und genau das war der Kernbefund dieses Tages ([Retro d57884](https://github.com/achimdehnert/platform/blob/main/docs/retros/session-retro-2026-08-15-platform-d57884.md)). Weiterhin **bewusst kein Issue** — der Melder soll nicht als erstes Reflex-Artefakt produzieren, was er selbst beanstandet.
