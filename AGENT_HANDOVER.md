...
4. **Zwei GATE-HEADER tragen noch das alte Fenster „bis 16.08." — Owner-Zug (selbstbetreffend).** `tools/claude-hooks/deferred_item_scanner.py` (Z. 16) und `scope_checkpoint_scanner.py` (Z. 17) müssten auf `2026-08-15 bis 2026-08-29` umdatiert werden. Der Permission-Classifier blockte die Änderung — beide Dateien sind Leitplanken für das Verhalten des Agenten selbst, eine Selbst-Umdatierung gehört nach Charta Art. 3 nicht in Agentenhand. Bewusst **nicht** halb ausgeführt: eine der beiden Änderungen ging durch und wurde zurückgenommen, um keinen inkonsistenten Stand zu hinterlassen.
