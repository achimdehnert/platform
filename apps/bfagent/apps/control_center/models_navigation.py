...
class NavigationSection(models.Model):
    ...
    # Section Link (optional - makes section header clickable)
    url_name = models.CharField(
        max_length=100,
        blank=True,
        help_text="Django URL name for section header link (e.g., 'control_center:workflow-v2-dashboard')"
    )
    external_url = models.URLField(
        blank=True,
        help_text="External URL for section header (if not using Django URL name)"
    )
    ...
    def get_url(self):
        """Get URL for section header (if configured)"""
