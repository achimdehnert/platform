...
def get_sections_for_domain(domain_slug, user=None):
    ...
    try:
        if use_new:
            ...
            from apps.bfagent.models_domains import DomainArt, DomainSection
...
def get_item_url(item, request=None):
    ...
    Returns:
        str: URL for the item (never raises, always returns valid string)
    """
    use_new = settings.NAVIGATION_FEATURES.get('USE_NEW_SCHEMA', False)
    
    try:
        if use_new:
            # NEW: NavigationItem - use model's get_url method if available
            if hasattr(item, 'get_url'):
                return item.get_url(request) or '#'
