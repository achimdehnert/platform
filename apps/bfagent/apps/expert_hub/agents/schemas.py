"""Pydantic Schemas für strukturierte Agent-Outputs."""
...
class SubstanceProperties(BaseModel):
    ...
    name: str = Field(description="Stoffname")
    cas_number: str | None = Field(default=None, description="CAS-Nummer")
    lower_explosion_limit: float = Field(description="UEG in Vol-%")
    upper_explosion_limit: float = Field(description="OEG in Vol-%")
    flash_point_c: float | None = Field(default=None, description="Flammpunkt in °C")
    ignition_temperature_c: float | None = Field(default=None, description="Zündtemperatur in °C")
    temperature_class: str | None = Field(default=None, description="Temperaturklasse (T1-T6)")
    explosion_group: str | None = Field(default=None, description="Explosionsgruppe (IIA/IIB/IIC)")
    vapor_density: float | None = Field(default=None, description="Dampfdichte relativ zu Luft")
    molar_mass: float | None = Field(default=None, description="Molare Masse in g/mol")
