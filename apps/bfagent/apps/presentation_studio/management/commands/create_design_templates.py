"""
Create system design templates
"""
...
class Command(BaseCommand):
    help = 'Creates default system design templates'
    ...
    def handle(self, *args, **options):
        ...
        for template_data in templates:
            ...
            if created:
                created_count += 1
                self.stdout.write(
                    self.style.SUCCESS(f'✓ Created template: {profile.profile_name}')
                )
            else:
                self.stdout.write(f'- Template already exists: {profile.profile_name}')
