...
This script identifies and removes obsolete and duplicate tools from the scripts directory.
It creates a backup of the files before deletion to ensure data safety.
...
def create_backup():
    """Create a backup directory for obsolete tools."""
    BACKUP_DIR.mkdir(parents=True, exist_ok=True)
    print(f"✅ Backup directory created: {BACKUP_DIR}")


def backup_and_delete(category, files):
    """Backup specified files and delete them from the scripts directory."""
    print(f"\n{'='*80}")
...
def generate_summary():
    """Generate a summary of files to be cleaned up."""
    total_deleted = 0
    total_missing = 0
