...
# ADR-100: Erweitertes Agent-Team — Cascade als Tech Lead, Deployment Agent, Review Agent und explizite Rollenentlastung
...
Eingehende Aufgabe
    ...
    │
    ├─ Typ: Feature / Bugfix
    │   ├─ complexity >= complex → Cascade plant → Developer führt aus
...
- [ ] `agent_team_config.yaml` um `deployment_agent` Rolle erweitern
- [ ] `orchestrator_mcp/agent_team/roles.py` — neue Rolle `DeploymentAgent`
- [ ] CD-Workflow (`.github/workflows/cd.yml`) als Deployment Agent konfigurieren
- [ ] `/deploy` Windsurf-Workflow auf Agent-Routing umstellen

### Phase 2 — Review Agent (kurzfristig)

- [ ] `orchestrator_mcp/agent_team/roles.py` — neue Rolle `ReviewAgent`
- [ ] GitHub Actions PR-Trigger-Workflow hinzufügen
- [ ] Neuen Windsurf-Workflow `/agent-review` erstellen

### Phase 3 — Workflow-Dokumentation (kurzfristig)

- [ ] `/agentic-coding` Workflow: Cascade-als-Tech-Lead explizit dokumentieren
- [ ] `/deploy` Workflow: Deployment Agent als primäre Ausführungsrolle
- [ ] `/pr-review` Workflow: Review Agent als erste Prüfstufe
...
### Negativ / Risiken
- Deployment Agent benötigt SSH-Key als GitHub Secret (bereits umgesetzt: `SSH_PRIVATE_KEY`)
- Review Agent kann False Positives produzieren — manuelle Override-Option nötig
- Initiale Setup-Kosten für `orchestrator_mcp`-Erweiterung

### Neutrale Änderungen
- `agent_team_config.yaml` muss in allen Repos aktualisiert werden
- Bestehende Workflows (`/deploy`, `/agentic-coding`) erhalten Breaking Changes — Versionierung nötig

---

## 7. Compliance

Dieses ADR operationalisiert und erweitert:
...
| ADR-075 | Deployment-Execution: jetzt als dedizierten Agent |
...
- Windsurf Workflows: `/deploy`, `/agentic-coding`, `/pr-review`
