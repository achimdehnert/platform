...
CREATE TABLE llm_calls (
    ...
    duration_ms     INTEGER,
    source          TEXT,        -- 'discord_chat', 'discord_ask', 'mcp_tool', etc.
...
Preise werden als Konfiguration gepflegt (OpenRouter-Preise, Stand 2026-03):
...
- Dashboard als JSON via Volume-Mount provisioniert (Infrastructure as Code)
...
- **Contra**: Mehr Custom-Code für dasselbe, kein Alerting, keine Time-Series out-of-the-box
...
- **Entscheidung**: Direkte PostgreSQL-Datasource reicht für diesen Use Case

### Externe SaaS (Datadog, New Relic)

- **Pro**: Kein Eigenbetrieb
- **Contra**: Kosten, Datenschutz, Lock-in
- **Entscheidung**: Self-hosted bevorzugt (ADR-Prinzip)
...
- Grafana Container braucht ~200MB RAM zusätzlich
- Preistabelle muss manuell aktualisiert werden wenn OpenRouter Preise ändert
- Qualitäts-Score (subjektiv) initial nicht automatisierbar

## Implementation

1. `llm_calls` Tabelle in orchestrator_mcp PostgreSQL (Migration in llm_mcp_service)
2. `llm_mcp_service/usage_logger.py` — Usage nach jedem Call in DB schreiben
3. `docker-compose.llm-mcp.yml` — Grafana Service hinzufügen
4. `grafana/provisioning/` — Datasource + Dashboard JSON
5. `llm_mcp_service/pricing.py` — Kosten-Kalkulation pro Model
