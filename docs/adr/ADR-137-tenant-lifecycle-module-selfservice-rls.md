...
implementation_evidence:
  ...
  - "Phase 4.2 RLS auf Prod+Staging aktiviert: DONE (2026-03-11)"
  - "4.2a setup_rls_roles: risk_hub_app (Prod) + risk_hub_staging_app (Staging) mit starken Passwörtern"
  - "4.2b enable_rls: 65 Tabellen mit tenant_isolation Policies (::uuid cast) — pg_policies verifiziert"
  - "4.2c Dual-DATABASE_URL: DATABASE_URL=app-user (RLS enforced), DATABASE_URL_MIGRATE=owner (RLS-exempt)"
  - "4.2d Entrypoint: migrate/seed nutzt Owner-Rolle, gunicorn nutzt App-User-Rolle"
  - "4.2e Secrets: Passwörter via .env (ADR-045 read_secret Chain), nicht hardcoded"
