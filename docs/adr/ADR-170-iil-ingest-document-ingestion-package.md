...
# ADR-170: iil-ingest — Reusable Document Ingestion Package (Tier 3)
...
**Neues Tier-3 Package `iil-ingest`** nach dem iil-enrichment-Pattern (ADR-169).
...
- **LLM-Klassifikation**: Nicht in v0.1 — Profile-Scoring reicht für bekannte
  Dokumenttypen. LLM-Extension möglich via `ClassifierProfile`-Plugin.
...
|---|---|---|
| 0 | Repo erstellen, pyproject.toml, CI | 30min |
...
| 7 | dms-hub: Migration auf iil-ingest | 2h |
...
- ADR-169: iil-enrichment (Pattern-Vorbild)
- ADR-163: Three-Tier Quality Standard (Tier-3 Package-Anforderungen)
- dms-hub: `apps/benefits/classifier.py`, `apps/accounting/extractor.py`
- iil-enrichment: `enrichment/provider.py`, `enrichment/registry.py`
