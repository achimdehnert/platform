...
**Selector-Konvention.** Bevorzugt stabile fachliche Anker (`data-testid`/`data-acceptance-id`); fragile CSS-/Text-Pfade werden markiert (Manifest-Warnung), ab Off-Ramp status-relevant (F18). Locator-Registry zurückgestellt (Doppelquell-Risiko, F18). *Rev 22 — F23 geschlossen (Hybrid, KONZ-iil-klickdummy-007):* `assert.selector` trägt ein optionales Präfix-Vokabular — `testid=` (→`get_by_test_id`), `role=…[name=…]` (→`get_by_role`), `label=` (→`get_by_label`) gelten als **stabile** Anker; `text=` (→`get_by_text`) ist der i18n-fragile, markierte Fallback; präfixlos bleibt CSS (`page.locator`, fragil). **Kein Schema-Bruch** — `selector` bleibt ein `string`. Am Off-Ramp härtet `--strict-selectors` die Manifest-Warnung zum Fehler (exit 3). Die **Locator-Registry bleibt zurückgestellt** (F18 unverändert) — der Hybrid härtet die Konvention, ersetzt sie nicht durch eine zweite Wahrheit.
...
- **Rev 22 (2026-06-30 — F23 geschlossen: Selektor-Kontrakt gehärtet, Hybrid)** — **Erweiterung, kein Entscheid-Widerruf**; `status` bleibt `accepted`. Pre-Check per `adr-threshold.md`: **härtet die bestehende §Selector-Konvention** (Rev 18) und schließt das offene **F23** — keine neue Boundary, **kein 5. Invariant (I5)**, kein eigener ADR-21X; die normative Änderung ist eine *Konventions-Härtung* (opt-in-Gate), org-weit nur **als Konvention** wirksam, nicht als Statusgate. **Entscheidung (ratifiziert):** der Hybrid aus `KONZ-iil-klickdummy-007` (T2) — **(D1)** der bisher rein *sichtbar machende* `is_fragile_selector`-Manifest-Hinweis wird am Off-Ramp *verhindernd*: `--strict-selectors` macht einen fragilen Selektor zum Fehler (exit 3) statt nur zur Warnung (opt-in, reversibel; reine Mockup-Läufe unverändert); **(D2)** `assert.selector` erhält ein Präfix-Vokabular `testid=`/`role=…[name=…]`/`label=` (stabile Anker) und `text=` (i18n-fragiler, markierter Fallback) → passende Playwright-`get_by_*`-API; präfixlos bleibt CSS (`page.locator`). **Kein Schema-Bruch** — `selector` bleibt `string`, Bestands-Specs (inkl. bare `[data-testid=…]`) bleiben bit-identisch gültig (Regressionstest); **(D3)** die **Locator-Registry bleibt zurückgestellt — F18 unverändert offen** (kein zweiter SoR ohne gefeuerten Cross-Repo-Refactor-Trigger). **Warum kein „B statt A":** die F23-Frage „data-testid-Konvention *oder* semantische Selektoren" war eine Scheinalternative — A ist bereits akzeptierter weicher Body (Rev 18) und der einzige real grüne E2E-Pfad (Rev 21); B wird als *Fallback* eingegliedert, nicht als Ersatz. **Empirie (lokal gefahren, „gemergt ≠ ausgeführt"-Lesson eingehalten):** `iil-klickdummy v1.29.0` (`_locator_expr`-Präfix-Dispatch + `--strict-selectors`, `manifest.strict_selectors`); **4 neue Tests** (Präfix-Dispatch, render_assertion-Routing, Fragilitäts-Klassifikation, Off-Ramp-Gate inkl. exit-3 *und* Default-0 *und* stabiler-Anker-grün), **151 Tests grün**, `ruff check` clean. **Ehrliche Reichweite:** das Vokabular wirkt erst nach **v1.29.0-Release (PyPI ausstehend)** in den Konsument-Repos; bis dahin nur im Generator-Repo aktiv. **F18 bleibt offen, F24 keine.** **PRs:** iilgmbh/iil-klickdummy #89 (KONZ-007), #90 (Implementierung, gemergt).
...
- **F18 (Selector-Fragilität / Locator-Registry zurückgestellt, Rev 18)**: die
  ...
  *Rev 22:* die in F18 angekündigte Härtung der Konvention ist als **F23
  geschlossen** (Präfix-Vokabular + `--strict-selectors`-Off-Ramp-Gate, Hybrid
  KONZ-007) — **F18 selbst bleibt offen**: die Registry-Frage ist *nicht*
  beantwortet, ihr Schließungs-Trigger (Cross-Repo-UI-Refactor) ist ungefeuert.
