...
- **Adversarial reviews:** sechs Cascade-Pässe (Rev 2/3/4/9 + Rev 10 F5-Rollback + Rev 11 #228/#229) + Rev-12-Empiriebasis (meiki-hub PR #23, 7 Iterationen 2026-05-20) + Rev-13-Decider-Pivot (ADR-214-Draft als advocatus diabolus zurückgezogen, 4 🔴-Findings) + Rev-14-Browser (Stakeholder-Feedback 2026-05-21) + Rev-25-Pre-Mortem (Rev-3-Review nach vier Monaten Empirie beantwortet), siehe Revisionshistorie
...
| `story` | (Storybook-Stil — kein Beispiel-Repo; *Rev 25* I1–I4-Mapping: `*.stories.*` = Spec, prod-isolierter Catalog-Build, Parity via visuellem Regressionstest z. B. Chromatic, Namensraum über die `*.stories.*`-Konvention) | synth. pro Story | **real**, isolierte Catalog-Route | Catalog-Route 404 |
...
# S15 Demo-Host statt Query-Parameter (Rev 25, optional, nicht status-gatend)
#     Pro Repo mit spec-demo/stub-demo: host | query | n-a. `host` = Demo läuft
#     auf demo-<app>.iil.pet hinter Cloudflare Access und `curl <prod>/?demo=x`
#     ist byte-gleich zu `curl <prod>/`. Bestand 2026-09-16: writing-hub query,
#     ausschreibungs-hub query, 0 host.
```
...
## §Alternatives Considered — Pre-Mortem (Rev 25, #230)

Die Frage aus dem Rev-3-Review: *„Wir warten sechs Monate. Was geht konkret
kaputt?“* — beantwortet nicht mit Prognose, sondern mit den vier Monaten, die
seit dem Accept (2026-05-19) tatsächlich vergangen sind (Stand 2026-09-16):

| Pre-Mortem-These (Rev 3) | Was eingetreten ist | Beleg |
|---|---|---|
| meiki/risk/writing bleiben bei eigenen Mustern | **Gegenteil:** 28 Repos führen `klickdummy/`, 91 `screens-spec.yaml` im gemeinsamen Schema; risk-hub allein 28 Specs | `ls -d $GITHUB_DIR/*/klickdummy`, `ls */klickdummy/*/screens-spec.yaml` |
| Kein Sicherheitsvorfall über `?demo=` | Zutreffend — aber F11 (externer Prod-Probe) ist dormant; Sicherheit beruht auf Selbstdeklaration (Rev 20) → Rev 25 verlagert sie auf den Host (I2) | Rev 20, #255 |
| Genau eine Drift-Episode | Zutreffend (`2026-05-19-klickdummy-adr180-collision`); I4 hat seither keine zweite zugelassen | Drift-Memory |
| „Policy + Skill hätte gereicht“ | **Widerlegt durch das, was Policy nicht tragen kann:** ein pip-Paket mit Schema und `check_i1..i4` (`iil-klickdummy` 1.41.1, Rev 15), drei Drift-Gates (S10/S13/S14), ein Prod-Sicherheits-Pattern (Rev 11) — alles Boundaries, keine Konventionen | §Distribution, Scoreboard |

**Ergebnis:** Der ADR ist gerechtfertigt; die Ausfallform des Wartens wäre nicht
ein Vorfall gewesen, sondern 28 Repos mit 28 privaten Schemata. Die Alternativen
des Rev-3-Reviews sind damit so entschieden (Owner 2026-09-16, „ja“ zu #224, #225,
#226, #227, #230):

| Alternative | Entscheid | Wo |
|---|---|---|
| Demo auf eigener Subdomain statt `?demo=` (#224) | **übernommen** als SOLL in I2, Bestand migriert opt-in | I2 Rev 25, S15 |
| Klickdummy als generiertes Artefakt (#225) | **seit Rev 13/15 erfüllt** — Renderer #1 ist Shell + Widget über `screens-spec.yaml`; hand-geschriebene `index.html` sind Bestand (Rev ≤12), ihr Off-Ramp ist das Entfernen der Datei | §Distribution, `/klickdummy` AK7 |
| Ein plattformweites Spec-Schema (#226) | **seit Rev 15 erfüllt** — `iil-klickdummy` liefert Schema, `check_i1..i4`, Renderer; repo-lokale Zusatzgates (z. B. risk-hub `verify_dummy.py` für Walkthroughs) sind Ergänzung, kein Ersatz | §Distribution |
| Storybook in der Auswahlhilfe (#227) | **übernommen** als Pattern `story` (Rev 11) mit explizitem I1–I4-Mapping (Rev 25); kein Beispiel-Repo — Adoption offen, kein Gate | §Auswahlhilfe |
| Pre-Mortem als Sektion (#230) | **übernommen** — diese Sektion | hier |
...
- **Rev 25 (2026-09-16 — Rev-3-Review-Vorschläge entschieden: Host statt Query, Pre-Mortem)** — **Erweiterung, kein Entscheid-Widerruf**; `status` bleibt `accepted`. Pre-Check per `adr-threshold.md`: keine neue Invariante, kein eigener ADR — I2 bekommt eine SOLL-Präferenz, der Rest ist Dokumentation bereits erfüllter Punkte. Anlass: fünf Folge-Issues des advocatus-diabolus-Reviews zu Rev 3 (#224–#227, #230) lagen vier Monate offen; Owner-Entscheid 2026-09-16 „ja“ zu allen fünf. **Neu:** (1) I2 *Host statt Query* — Demo-Zustände SOLLEN auf `demo-<app>.iil.pet` hinter Cloudflare Access laufen statt als `?demo=` auf der Prod-Oberfläche; Abnahmetest `curl <prod>/?demo=x` ≡ `curl <prod>/`; Scoreboard **+S15** (Bestand: writing-hub, ausschreibungs-hub = `query`). (2) §Alternatives Considered mit dem beantworteten Pre-Mortem (28 Repos, 91 Specs, 1 Drift-Episode in vier Monaten). (3) `story`-Zeile der Auswahlhilfe mit explizitem I1–I4-Mapping. **Bereits erfüllt, nur festgehalten:** #225 (Renderer #1 ist spec-gerendert seit Rev 13/15) und #226 (ein Schema via `iil-klickdummy`, Rev 15). **Kein neues Gate.** Schließt #224, #225, #226, #227, #230.
