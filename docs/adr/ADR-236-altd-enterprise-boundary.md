...
> Status **`accepted`** (2026-06-03; Kill-Gate a/b/c erfüllt — a/b owner-attestiert schriftlich, c bewiesen). Dieser ADR ist die *Boundary-Entscheidung*; die **Ausführung** ist phasenweise gegated (Runbook) — `implementation_status: partial` seit **S1 ausgeführt 2026-06-03** (4 Member-Orgs, Seats 2/2, live verifiziert). S2–S4 offen.
...
3. **Government-Orgs `ttz-lif`/`meiki-lra`** — **Amendment 2026-06-03:** ~~bleiben standalone~~ → **bewusst in die Enterprise aufgenommen** (`exit_class: central-gov`). Möglich geworden durch einen **formalen Träger-Souveränitäts-Sign-off, der volle Enterprise-Mitgliedschaft deckt** (owner-attestiert, DR-B). Damit erben sie die Enterprise-Security-Posture direkt (kein Mirror/CI-Gleichheits-Audit mehr nötig). **Bedingung:** sie müssen rückgabefähig bleiben (Exit = Übergabe an Träger) → keine nicht-exportierbaren Kopplungen (`central-gov`-`forbidden_features`); Souveränität ruht jetzt als **Single Point** auf Scope + Bestand des Sign-offs. *Damit verschiebt sich die Topologie-Wahl faktisch von ALT-D (GOV standalone) zu **ALT-C (Voll-Konsolidierung), dessen Souveränitäts-Blocker der Sign-off aufhebt** — siehe §3.*
   > **⚠️ RAHMENKORREKTUR 2026-06-04 (Owner-Datenprämisse) — überschreibt diesen Punkt:** `ttz-lif`/`meiki-lra` sind **Kundenprojekte** (Landratsämter Traunstein/Günzburg) mit nur **Code + Demo-Daten, KEINEN realen Personendaten**. Reale Daten + Datensouveränität entstehen erst **post-Übergabe im Kunden-RZ** (außerhalb GitHub); Übergabe = Code-Portierung, **kein** GitHub-Org-Transfer. ⟹ Es findet **keine Verarbeitung realer Personendaten in GitHub** statt → **kein AVV/keine Souveränitäts-Gating auf GitHub-Ebene**; die `central-gov`/Sign-off-/Single-Point-Konstruktion ist **gegenstandslos**. Reklassifiziert auf **`exit-likely`** (wie `bahn-sqf`), `teardown_authority: self-owner`, `central-gov` retired (`governance/exit-classes.yaml`). **Leitplanke:** nur Demo-Daten in diese Repos (Push-Protection schützt zusätzlich).
...
| Risiko | Mitigation |
|---|---|
| **Souveränitätsbruch** (B4) | **Amendment 2026-06-03:** GOV nun *in* der Enterprise (`central-gov`), abgesichert durch **formalen Träger-Sign-off** (owner-attestiert) statt Standalone-Trennung. **Neues Rest-Risiko:** Souveränität ist jetzt *Single Point* am Sign-off — zu überwachen sind dessen **Scope** (deckt volle Mitgliedschaft) und **Bestand** (Widerruf → Rückgabepflicht). Mitigation: `central-gov` verbietet nicht-exportierbare Kopplungen (rückgabefähig). **GOV-Exit-Feuerübung 2026-06-03 teilweise gelaufen** ([Plan](../runbooks/KONZ-002-gov-exit-firedrill.md); Protokoll `~/shared/gov-exit-drill-protocol-2026-06-03.md`): Inventar grün (Secret wird beim Transfer entfernt, Variable überlebt — bestätigt), **aber Org-Exit/Teardown NICHT autonom-programmatisch** (create=API-404, delete=Owner/Web-UI) → **Souveränitäts-Rückgabe braucht vertragliche/operative SLA**, kein Skript. Voll-grün erst nach echter Owner-Handover-Probe. **⚠️ Rahmenkorrektur 2026-06-04:** entschärft — GOV-Repos halten nur Code+Demo-Daten (keine realen Personendaten); reale Souveränität liegt im Kunden-RZ post-Übergabe. Damit ist „Souveränitätsbruch" auf GitHub-Ebene **kein offenes Risiko** mehr; Restpunkt ist nur die **Leitplanke „nur Demo-Daten"** + REC-9-Exit-Kosten (wie jedes exit-likely-Kundenprojekt). |
...
- 2026-06-03: **GOV-Exit-Feuerübung (teilweise).** Read-only Drill auf Wegwerf-Org `gov-exit-drill` (Protokoll in `~/shared/`): H4 bestätigt (Secret entfernt/Variable überlebt); **H1-Hauptbefund:** Org-Exit/Teardown ist NICHT autonom-programmatisch (create=API-404, delete=Owner-Web-UI/safety-gated) → Souveränitäts-Rückgabe braucht **SLA**, kein Skript (§6 geschärft). Voll-grün erst nach echter Owner-Handover-Probe. **Offener loser Faden:** `gov-exit-drill`-Org per Web-UI löschen.
- 2026-06-03: **S3 Canary** (`desktop-setup` → `iilgmbh`) erfolgreich. **§2.2 verfeinert:** transferierte Repos erben Config 17 (Default-for-new) — für Code-Repos **erwünscht** (CodeQL = eigene Verteidigungslinie); nur **Nicht-Code-Repos** gezielt schlank überschreiben (Org-Level-Attach `scope=selected`). Damit ist „schlank" kein blanket-Default mehr, sondern repo-typ-abhängig (Detail: Runbook `KONZ-002-s3-repo-transfer.md`). Befund: Enterprise-Attach = nur `all`; per-Repo-Override via Org-Level-Endpoint.
