...
- Vorgang schließen: `python3 tools/mail_agent/board.py --erledigt <nr> [--am YYYY-MM-DD] [--grund '…']` setzt `bucket: erledigt`, `erledigt_am`, `zustand` und den Verlaufseintrag in einem Zug; Gegenstück `--wiedereroeffnen <nr>` ([#3049](https://github.com/achimdehnert/platform/issues/3049)). Seit V3 ([#3015](https://github.com/achimdehnert/platform/issues/3015) K4) verlangt `--erledigt` einen Anker: fehlt er, wird die jüngste Mail-Referenz aus dem Verlauf automatisch verankert; gelingt das nicht, bricht das Kommando mit Exit 1 und einem fertigen `anker.py --setze …`-Vorschlag ab — `--ohne-anker` erzwingt den Abschluss trotzdem und vermerkt das im Verlauf.
- Vorgang aus einer Mail anlegen: `python3 tools/mail_agent/board.py --neu <konto> <ORDNER>#<UID> --typ <TYP> [--bucket owner|warten] [--frist YYYY-MM-DD|keine --grund '…'] [--kurz '…']` (V2, [#3015](https://github.com/achimdehnert/platform/issues/3015) K4) holt Von/Betreff/Datum/Message-ID per IMAP (hnu/ad) und verankert die Mail gleich mit. Für iil (kein IMAP) bzw. manuell: `--von <adresse> --betreff '…' --datum YYYY-MM-DD` statt IMAP-Zugriff.
...
| 7 | Secret-Prüfhelfer (Hash, HTTP, Formprüfung) | Der Guard hat heute korrekt gefeuert, das Leck kam vom Sourcing, nicht vom Lesen — ein weiterer Lese-Guard hätte nichts geändert | Werte gar nicht mehr lokal halten, nur Hashes; der echte Test läuft im Zielcontainer, nicht auf dem Client | offen (PR folgt) |
## Modellfest-Drill (K5)
