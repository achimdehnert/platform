...
**Gate-Status (2026-06-03, Stufe 2):** **alle drei Kill-Gate-Bedingungen erfüllt.** (c) ✅ bewiesen (§15, PR #429; Reversibilität D1-„dokumentiert", Einbahn-Exit-Befund). (a) ✅ **schriftliche Kostenbestätigung** liegt vor (DR-A) und (b) ✅ **formaler Träger-Sign-off** liegt vor (DR-B) — beide **owner-attestiert**, in Privatunterlagen abgelegt (*attestiert ≠ repo-verifiziert*; Beleg bei Audit nachreichbar). **GOV-Hard-Lock aufgehoben.** Verbleibende Sperre: nur `pactive-de` (Dritt-Org-Eigentum, kein Gate-Thema). Boundary-ADR = **ADR-236** (PR #431, `proposed`). Operativer Plan: [`docs/runbooks/KONZ-002-consolidation-rollout.md`](../runbooks/KONZ-002-consolidation-rollout.md). **Offen vor S1:** ADR-236 → `accepted` (Governance-Schritt) + `exit_class`-SSoT (PR #432). Danach: Owner führt S1 (Web-UI) aus.
