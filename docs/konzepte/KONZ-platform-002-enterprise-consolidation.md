...
**Gate-Status (2026-06-03):** (c) ✅ bewiesen (§15, PR #429). (a)/(b) **bewusst via Budget-Owner-Aktennotiz** geschlossen (nicht still — DR-A/DR-B + ADR-235-Amendment), Restrisiko benannt: (a) €-Sätze extern unbestätigt, (b) *mündliches OK protokolliert, kein formaler Träger-Sign-off*. Owner hat den Rollout für die **reversiblen/eigenen** Phasen freigegeben. **Harte Sperren unverändert:** GOV-Ownership/-Transfer bis schriftlicher Sign-off (ALT-D verlangt es nicht — GOV bleibt standalone); `pactive-de` (Dritt-Org-Eigentum). Operativer Plan: [`docs/runbooks/KONZ-002-consolidation-rollout.md`](../runbooks/KONZ-002-consolidation-rollout.md). `pipeline_status` bleibt `idea` bis zum ALT-D-Boundary-ADR (nächster Governance-Schritt vor S1).
