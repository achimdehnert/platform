...
|---|---|---|
| K1 Positivkontrolle: >= 7 der 9 bekannten Defekte (writing-hub 3, ausschreibungs-hub 6) wiedergefunden **je am Commit vor dem eigenen Fix** (E19) | teilweise (**5/9**), Nenner **9 bestaetigt** (Owner 2026-08-30, Weg A — [#2474](https://github.com/achimdehnert/platform/issues/2474)); Messung wird gegen die E19-Staende wiederholt | writing-hub 3/3 am 2026-08-25, Stand 493dc19 — writing-hub#767. ausschreibungs-hub 2026-08-30, Stand 8c8090e: **2 gefunden** (markup-leck auf zwei Seiten; beide unbegehbaren Stationen `angebote:review` und `submission_workflow`), **2 verfehlt** (CSRF-403 an „Analyse starten", eine Meldung fuer drei Ursachen — beide hinter der Analyse-Tuer, R8), **2 waren am Stichtag bereits behoben** (`docx`-Extraktor registriert, `docx`/`excel`-Extra in `pyproject.toml`) und konnten dort gar nicht auftreten |
