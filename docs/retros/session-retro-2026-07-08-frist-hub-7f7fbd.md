...
- **`AGENT_HANDOVER.md` ist auf `main` stale** — 3 PRs (#18, #20, #22) wurden nach dem letzten
  ...
  frist-hub bindet den reusable `platform:handoff-banner-gate.yml` bereits ein (PR #21) und
  erbt automatisch dessen Rezenz-Check-Upgrade (platform PR #981, 2026-07-07, aus Retro
  2026-07-06/9206ac hervorgegangen) — der Trigger bleibt aber `paths: AGENT_HANDOVER.md`,
  reagiert also strukturell nicht auf PRs, die daran vorbei mergen.
...
| 2 | frist-hub bindet den reusable `platform:handoff-banner-gate.yml` bereits ein (`.github/workflows/handoff-banner-gate.yml`, `uses: achimdehnert/platform/...@main`, aus PR #21) und erbt damit automatisch platform PR #981s dialektunabhängigen Rezenz-Check — trotzdem konnte der Gate #1 nicht fangen, weil der Trigger unverändert `on.pull_request.paths: AGENT_HANDOVER.md` ist (verifiziert in `platform/.github/workflows/handoff-banner-gate.yml:29-35`) und damit auf PRs, die die Datei nicht berühren (#18/#20/#22), strukturell nie feuert. **Korrektur ggü. zwei Vorversionen dieses Befunds:** weder „Gate fehlt konzeptionell" noch „Caller fehlt in frist-hub" trifft zu — Caller UND verbesserte Check-Logik existieren bereits, nur der Trigger-Scope bleibt zu eng. | Gate-Design (Trigger-Scope) | mittel (von hoch herabgestuft) | SURVIVES (zweifach korrigiert) | `git log --oneline -- .github/workflows/handoff-banner-gate.yml` (frist-hub, zeigt PR #21), Dateiinhalt beider `handoff-banner-gate.yml` (frist-hub Caller + platform Reusable) gelesen | neu — direkt an #1 gekoppelt |
...
| frist-hubs eigene Caller-Datei (`.github/workflows/handoff-banner-gate.yml`) triggert nur auf `paths: AGENT_HANDOVER.md` — dieser Trigger ist frist-hub-lokal definiert und unabhängig vom platform-Reusable-Workflow editierbar. | `on.pull_request.paths` in frist-hubs eigener Caller-Datei um `apps/**`, `docs/adr/**`, `docs/klickdummy*/**` erweitern — reine frist-hub-lokale Änderung, keine Abhängigkeit von platform. | #2 |
...
| 5 | `on.pull_request.paths` in frist-hubs eigener Caller-Datei (nicht platform) um `apps/**`, `docs/adr/**`, `docs/klickdummy*/**` erweitern | frist-hub | — | 🔵 ready | ich: Workflow-Patch vorbereiten, PR öffnen |
