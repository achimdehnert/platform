...
recurring_findings: [claim-before-cheapest-check, partial-fix-not-generalized-to-sibling-artifacts, deferred-item-no-tracking-issue, handover-stale-vor-merge, scope-checkpoint-not-durably-recorded, same-file-serial-prs]
...
| 7 | PR #290 eröffnet mit Commit-Message, die einen Handover-Diff behauptete, der fehlte; 67 s später Amend + Force-Push (`head_ref_force_pushed` 06:54:20Z); dazu #263-Schließkommentar zunächst mit Tool-Fehlermeldung („aborted: you must first push…"), 41 s später korrigiert | Werkzeug | mittel | SURVIVES (Finder-Konflikt per Timeline entschieden) | `gh api …/issues/290/timeline` → head_ref_force_pushed; `gh pr view 263 --json comments` 06:20:42/06:21:23 | chained-shell-continues-after-block-failure (neu) |
...
| code_konventionstreue | 4 | Service-Layer eingehalten und in operations verbessert; 26+9+5 Tests; Commit-Format; `{% load %}` vor DOCTYPE bereinigt |
...
| `scope-checkpoint-not-durably-recorded` (advisory, 2026-08-23, zu-frueh) | 1× | #4 | Beobachtung fortführen; Fall ist die Bypass-Variante (Freigabe für Security-Config nur im Chat) — als Marker-Familie „--admin/--force/bypass" in den Scope-Hook aufnehmen |
