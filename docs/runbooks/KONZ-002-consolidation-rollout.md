...
| **S2** | **Schlanke** Config (Secret-Scan+Push-Protection) apply-to-all `unenforced`→`enforced`; Config 17 = Default-for-new (§2.2) — Detail: [`KONZ-002-s2-config-rollout.md`](./KONZ-002-s2-config-rollout.md) | S1 grün; FP-Rate + blockierte Pushes + Coverage ausgewertet | Owner/CC (PAT admin:enterprise) | apply-to-all `enforced`; Drift-Audit grün; FP-Rate akzeptabel | detach / `unenforced` / Config löschen |
