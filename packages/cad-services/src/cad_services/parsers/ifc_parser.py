    ...
    def parse(self, file_path: Path) -> CADParseResult:
        try:
            ...
            raise CADParseError(
                ...
                message="ifcopenshell ist nicht installiert (installiere Extras: cad-services[ifc])",
        ...
        try:
            ...
            raise CADParseError(code="FILE_HASH_FAILED", message="Konnte Datei nicht hashen", file_path=str(file_path)) from e
        ...
        statistics = CADParseStatistics(
            ...
            ifc_schema=getattr(getattr(ifc, "schema", None), "name", None) or getattr(ifc, "schema", None),
        )
