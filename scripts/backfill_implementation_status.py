...
STATUS_MAP: dict[int, tuple[str, list[str]]] = {
    ...
    44: (
        IMPLEMENTED,
        ["mcp-hub: consolidated architecture (deployment, orchestrator, llm MCPs)"],
    ),
    ...
    97: (
        IMPLEMENTED,
        [
            "aifw/src/aifw/: models, migration 0005, service, constants, types, admin, tests"
        ],
    ),
    ...
    85: (
        PARTIAL,
        ["mcp-hub/orchestrator_mcp/: basic task pipeline, NL→TaskGraph pending"],
    ),
    ...
    96: (
        NONE,
        [
            "authoringfw: package exists but no writing/research/analysis sub-modules yet"
        ],
    ),
    117: (
        NONE,
        [
            "weltenfw: package exists but shared world layer not extracted from weltenhub yet"
        ],
    ),
    119: (
        NONE,
