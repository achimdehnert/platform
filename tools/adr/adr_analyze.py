...
import json, re, sys, collections, datetime, os

rows = json.load(open(sys.argv[1]))
TODAY = datetime.date.today()
# Vokabular = iil-adrfw Schema v3 (Fehlermeldung von `iil-adrfw validate`, 2026-07-04)
VOCAB = {"draft", "proposed", "accepted", "deprecated", "superseded", "rejected", "experimental", "void"}
...
for repo, rs in sorted(by_repo.items()):
    f = {"n": len(rs), "no_fm": [], "no_status": [], "bad_status": [], "no_date": [],
         "stale_proposed": [], "template_rest": [], "dup_nums": [], "tiny": []}
    ...
    for r in rs:
        ...
        if not r.get("has_fm"): f["no_fm"].append(fn)
        st = (r.get("status") or "").strip().lower().split()[0] if r.get("status") else ""
        if not st: f["no_status"].append(fn)
        elif st not in VOCAB: f["bad_status"].append(f"{fn} -> {r['status']!r}")
        if not r.get("date"): f["no_date"].append(fn)
        d = pdate(r.get("date"))
        if st == "proposed" and d and (TODAY - d).days > 90:
            f["stale_proposed"].append(f"{fn} ({r['date']}, {(TODAY-d).days}d)")
        if r.get("template_rest"): f["template_rest"].append(fn)
        if r.get("bytes", 0) < 400: f["tiny"].append(f"{fn} ({r['bytes']}B)")
...
for repo, rs in by_repo.items():
    ...
    for r in rs:
        ...
        for field, val in (("superseded_by", r.get("superseded_by")), ("supersedes", r.get("supersedes"))):
            ...
            for tn in re.findall(r"ADR-?(\d+)", val):
                ...
                tgt = nums.get(tn)
                if tgt is None:
                    broken.append(f"{repo}/{r['file']}: {field}=ADR-{tn} existiert nicht in {repo}")
                elif field == "supersedes" and (tgt.get("status") or "").lower() not in ("superseded", "deprecated", "void"):
                    broken.append(f"{repo}/{r['file']}: supersedes ADR-{tn}, aber Ziel-status={tgt.get('status')!r} (nicht superseded)")
...
for r in rows:
    ...
    if len(n) > 12:
        titles[n].add(r["repo"])
rep["phase2"]["cross_repo_title_dups"] = {t: sorted(rs) for t, rs in titles.items() if len(rs) > 1}

json.dump(rep, open(sys.argv[2], "w"), ensure_ascii=False, indent=1)

# Summary
p1 = rep["phase1"]
def total(key): return sum(len(v.get(key, [])) for v in p1.values())
print(f"repos={len(p1)} files={sum(v['n'] for v in p1.values())}")
for k in ("no_fm", "no_status", "bad_status", "no_date", "stale_proposed", "template_rest", "dup_nums", "tiny"):
    print(f"{k}: {total(k)}")
