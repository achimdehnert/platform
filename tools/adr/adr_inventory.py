...
FM_KEYS = ("status", "date", "supersedes", "superseded_by", "superseded-by", "impl", "implementation_status", "title")
        ...
        if end != -1:
            for line in text[3:end].splitlines():
                ...
                if m and m.group(1).lower() in FM_KEYS:
                    fm[m.group(1).lower().replace("-", "_")] = m.group(2).strip().strip('"').strip("'")
    return fm
...
def first_heading(text):
    ...
    m = re.search(r"^#\s+(ADR-\d+.*)$", text, re.M) or re.search(r"^#\s+(.+)$", text, re.M)
...
for path in sorted(glob.glob(f"{GH_DIR}/*/docs/adr/*.md")):
    ...
    rows.append({
        ...
        "madr": bool(re.search(r"^#{1,3}\s*(Context|Kontext)", text, re.M | re.I))
                and bool(re.search(r"^#{1,3}\s*(Decision|Entscheidung)", text, re.M | re.I)),
        "template_rest": bool(re.search(r"\{\{|TODO: FILL|Lorem ipsum|<platzhalter>", text, re.I)),
    })
