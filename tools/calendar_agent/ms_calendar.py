...
def load_config() -> dict:
    if not CONFIG_FILE.exists():
        sys.exit(
            f"FEHLER: {CONFIG_FILE} fehlt — Maschine ist für Kalender nicht freigegeben (Capability-Profil)"
        )
    cfg = parse_env(CONFIG_FILE)
    accounts = [
        a.strip() for a in cfg.get("GRAPH_ACCOUNTS", "").split(",") if a.strip()
    ]
    if not accounts:
...
def ics_events(url: str, win_start: dt.datetime, win_end: dt.datetime):
    ...
    for block in r.text.split("BEGIN:VEVENT")[1:]:
        ...
        if "DTEND" in props:
            ...
            dur = end_dt - start
        title = (
            props.get("SUMMARY", "(ohne Titel)")
            .replace("\\,", ",")
            .replace("\\n", " · ")
        )
        loc = props.get("LOCATION", "").replace("\\,", ",")
        rrule = dict(
            p.split("=", 1) for p in props.get("RRULE", "").split(";") if "=" in p
        )
        freq = rrule.get("FREQ", "")
        ...
        bydays = {
            WEEKDAYS[d] for d in rrule.get("BYDAY", "").split(",") if d in WEEKDAYS
        } or {start.weekday()}
        occ, made, day = 0, 0, start
        while day < win_end and made < 500:
            fits_interval = (
                freq == "DAILY" and ((day - start).days % interval == 0)
            ) or (freq == "WEEKLY" and (((day - start).days // 7) % interval == 0))
...
def cmd_login(cfg: dict, account: str) -> None:
    if ics_path(cfg, account).exists():
        sys.exit(
            f"{account} läuft über den Abo-Link (ICS) — keine Anmeldung nötig/möglich."
        )
    ...
    r = requests.post(
        f"{base}/devicecode", data={"client_id": client_id, "scope": SCOPES}, timeout=20
    )
    if r.status_code != 200:
        sys.exit(
            f"FEHLER: Device-Code-Anfrage {r.status_code} — {r.json().get('error_description', '')[:200]}"
        )
    dc = r.json()
    print(f"\n>>> Anmeldung für {account}:")
    print(f">>> {dc['message']}\n")
    print(
        "(Warte auf Bestätigung im Browser — MFA der Hochschule/Firma funktioniert hier normal.)"
    )
    ...
    while time.time() < deadline:
        ...
        if tr.status_code == 200:
            ...
            print(
                f"OK: angemeldet als {who.get('userPrincipalName', account)} — Token gespeichert (nur lesend, Calendars.Read)"
            )
            return
        if body.get("error") in ("authorization_pending", "slow_down"):
            continue
        sys.exit(
            f"FEHLER: {body.get('error')} — {body.get('error_description', '')[:200]}"
        )
...
def cmd_status(cfg: dict) -> None:
    for acc in cfg["accounts"]:
        ...
        print(
            f"{acc}: {'✔ angemeldet' if tok else '✘ nicht angemeldet — --login ' + acc}"
        )
...
def cmd_list(cfg: dict, days: int) -> None:
    ...
    for acc in cfg["accounts"]:
        ...
        if not tok:
            print(
                f"({acc}: nicht angemeldet — überspringe; --login {acc})",
                file=sys.stderr,
            )
            continue
        tag = (
...
def cmd_create(cfg: dict, account: str, args) -> None:
    ...
    body = build_event_body(
        args.subject, args.start, args.end, args.location or "", args.note or ""
    )
    ...
    if r.status_code not in (200, 201):
        sys.exit(
            f"FEHLER: Anlegen fehlgeschlagen HTTP {r.status_code} — {r.text[:200]}"
        )
    ev = r.json()
    print(
        f"OK: Termin angelegt — '{ev.get('subject')}' am {args.start} (nur dein Kalender, niemand eingeladen)"
    )
...
def main() -> None:
    ...
    g.add_argument(
        "--login", metavar="KONTO", help="einmalige Geräte-Anmeldung für ein Konto"
    )
