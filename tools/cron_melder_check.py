...
tragen die Zeile `# ROT-IST-BEFUND` in ihrer YAML-Datei und werden hier
übersprungen; sie brauchen Triage, nicht Reparatur.
...
Ausgabe endet auf `RESULT: OK|BEFUND|UNGEPRUEFT — <text>` (maschinenlesbar für
`tools/session_start_checks.sh`, Phase 0.7.2).
"""
...
def geplante_workflows(workflow_dir: Path) -> dict[str, Path]:
    """{Workflow-Name: Datei} für alle Workflows mit `schedule:`-Trigger.
    ...
    """
    aus: dict[str, Path] = {}
    for datei in sorted(workflow_dir.glob("*.yml")) + sorted(workflow_dir.glob("*.yaml")):
        text = datei.read_text(encoding="utf-8", errors="replace")
        if "schedule:" not in text:
            continue
        if MARKER_ROT_IST_BEFUND in text:
            continue
        if m := _NAME_RE.search(text):
            aus[m.group(1).strip().strip("\"'")] = datei
    return aus
...
def _hole_runs(repo: str, branch: str, workflow_datei: str, limit: int) -> list[dict]:
    ...
    roh = subprocess.run(
        ["gh", "api", "-X", "GET",
         ...
         "-f", "event=schedule", "-f", f"branch={branch}", "-f", f"per_page={limit}",
         "--jq", ".workflow_runs[] | {name, status, conclusion, created_at, html_url}"],
        capture_output=True, text=True, timeout=30,
    )
    if roh.returncode != 0:
        raise RuntimeError((roh.stderr or "gh-Aufruf fehlgeschlagen").strip().splitlines()[-1])
...
def main() -> int:
    ...
    ap.add_argument("--schwelle", type=int, default=3,
                    help="ab wie vielen roten Läufen in Folge gemeldet wird (Default 3)")
    ap.add_argument("--limit", type=int, default=10, help="wie viele Läufe je Workflow geholt werden")
    ...
    wf_dir = Path(args.workflow_dir).expanduser() if args.workflow_dir else (
    ...
    geplant = geplante_workflows(wf_dir)
    if not geplant:
        print(f"RESULT: OK — keine geplanten Workflows in {wf_dir.name} (ohne ROT-IST-BEFUND-Marker)")
        return 0

    befunde: dict[str, dict] = {}
    ungeprueft: list[str] = []

    def _einer(datei: Path):
        return datei, _hole_runs(args.repo, args.branch, datei.name, args.limit)
    ...
    with ThreadPoolExecutor(max_workers=8) as pool:
        for fut in as_completed([pool.submit(_einer, d) for d in geplant.values()]):
            try:
                _, runs = fut.result()
                ...
                continue
            befunde.update(rote_serien(runs, args.schwelle))

    if not args.quiet:
        print(f"Geprüft: {len(geplant) - len(ungeprueft)}/{len(geplant)} geplante Workflow(s) "
              f"auf {args.repo}@{args.branch}, Schwelle {args.schwelle}")
        for name, b in sorted(befunde.items()):
            print(f"  ROT  {name} — {b['serie']} Läufe in Folge, seit {b['seit']}, "
                  f"zuletzt {b['letzter']}")
            print(f"       {b['url']}")
        for u in ungeprueft:
            print(f"  ??   {u}")
    ...
    if befunde:
        kurz = "; ".join(f"{n} ({b['serie']}x seit {b['seit']})" for n, b in sorted(befunde.items()))
        print(f"RESULT: BEFUND — blinde(r) Melder: {kurz} — ein dauerhaft roter Melder "
              f"meldet nichts mehr (platform#1508){rest}")
        return 0
    if ungeprueft:
        print(f"RESULT: UNGEPRUEFT — {len(ungeprueft)} von {len(geplant)} nicht abrufbar, "
              "in den uebrigen kein dauerhaft roter Melder")
        return 0
    print(f"RESULT: OK — {len(geplant)} geplante Workflow(s) geprüft, keiner dauerhaft rot")
    return 0
