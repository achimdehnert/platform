...
EXIT_CLASSES = (
    Path(__file__).resolve().parent.parent / "governance" / "exit-classes.yaml"
)
...
def load_exit_classes(path: Path):
    ...
    try:
        ...
        return (
            None,
            "pyyaml not installed (`pip install pyyaml`) — policy section skipped",
        )
    try:
        with open(path) as f:
...
def main() -> int:
    ...
    st_org, org_obj = api(
        f"/repos/{org}"
    )  # cheap existence probe is per-repo; use orgs
    ...
    w()
    w(
        "> Generated from LIVE GitHub state (KONZ-platform-002 OOTB-4). "
    ...
    if ec_err:
        ...
        w(
            f"- ⚠️ {ec_err} — exit_class/placement/teardown_authority for `{org}` UNKNOWN."
        )
    else:
        ...
        if not org_pol:
            gaps.append(
                f"Org `{org}` not in exit-classes SSoT — "
                "exit_class/placement/teardown_authority undefined (ADR-236 §2.5)"
            )
            w(
                f"- ⚠️ `{org}` is **not** in the exit-classes SSoT → exit_class undefined. "
            ...
            w(
                f"- **exit_class:** `{klass}` — {(cdef.get('description') or '').strip() or 'no description'}"
            )
            w(f"- **placement:** `{cdef.get('placement', '?')}`")
            w(f"- **teardown_authority:** `{ta}`")
            rc = cdef.get("required_checks", [])
            w(
                f"- **required_checks:** {', '.join('`' + c + '`' for c in rc) or 'none'}"
            )
            forb = cdef.get("forbidden_features", [])
            if forb:
                w(
                    f"- **forbidden_features** (must NOT exist here): {', '.join('`' + c + '`' for c in forb)}"
                )
            ...
            if cdef.get("teardown_authority_rule") == "not-none" and ta == "none":
                gaps.append(
                    f"INVARIANT VIOLATED: `{org}` is `{klass}` with teardown_authority=none "
                    "(ADR-236 §2.5 forbids `none` for exit-likely/must-stay-local)"
                )
                w(
                    f"- 🛑 **INVARIANT VIOLATED:** `{klass}` requires teardown_authority ≠ none, but it is `none`."
                )
    ...
    def section(title, st, payload, extract):
        if st in (401, 403):
            gaps.append(title)
            w(
                f"- **{title}:** ⚠️ UNREADABLE with this token — verify manually before exit."
            )
            return
        vals = extract(payload) if payload else []
        if vals:
            w(
                f"- **{title}** ({len(vals)}): {', '.join('`' + str(v) + '`' for v in vals)}"
            )
        else:
            w(f"- **{title}:** none")

    section(
        "Org Actions secrets (names only — VALUES never transfer, rotate+re-add)",
        st,
        org_secrets,
        lambda d: names(d.get("secrets", [])),
    )
    section(
        "Org Actions variables", st_v, org_vars, lambda d: names(d.get("variables", []))
    )
    section(
        "Org webhooks",
        st_h,
        org_hooks,
        lambda d: [h.get("config", {}).get("url", "?") for h in (d or [])],
    )
    section(
        "Installed GitHub Apps",
        st_i,
        installs,
        lambda d: [a.get("app_slug", "?") for a in d.get("installations", [])],
    )
    ...
    if st_rl in (401, 403):
        gaps.append(
            "Repository listing UNREADABLE (token lacks `repo` scope) — "
    ...
    if expected and len(repos) < expected:
        gaps.append(
            f"Repository listing INCOMPLETE: saw {len(repos)} of ~{expected} "
            f"(token scope / visibility) — re-run with a `repo`-scoped token"
        )
    w(
        f"## 2. Repositories ({len(repos)}"
        + (f" of ~{expected} expected ⚠️" if expected and len(repos) < expected else "")
        + ")"
    )
    w()
    w(
        "Repo transfer keeps history + variables + sets up redirects, but **secrets "
    ...
    for r in sorted(repos, key=lambda x: x["name"]):
        ...
        env_names = (
            names(envs.get("environments", [])) if isinstance(envs, dict) else []
        )
            ...
            for f in wf:
                if f["name"].endswith((".yml", ".yaml")):
                    ...
                    if (
                        ...
                        and OWNER_REF_HINT in body
                    ):
                        owner_ref_files.append(f["name"])
        w(
            f"- secrets (**REMOVED on transfer** — rotate+re-add): {', '.join('`' + s + '`' for s in sec_names) or 'none'}"
        )
        w(
            f"- variables (transfer WITH repo — verify): {', '.join('`' + s + '`' for s in var_names) or 'none'}"
        )
        w(f"- environments: {', '.join('`' + s + '`' for s in env_names) or 'none'}")
        w(
            f"- webhooks: {hook_n} · deploy keys: {key_n} · rulesets: {rs_n} · "
            f"pages: {'yes' if st_pg < 400 else 'no'} · "
            f"code-scanning default-setup: {cs.get('state', '?') if isinstance(cs, dict) else 'n/a'}"
        )
        if owner_ref_files:
            w(
                f"- ⚠️ **hardcoded `{OWNER_REF_HINT.decode()}*` refs** (break on owner change → "
    ...
    w()
    w(
        "- **OIDC cloud trusts:** AWS/GCP/Azure role trust policies that reference "
        "`repo:%s/*` — update on the cloud side, not in GitHub." % org
    )
    w("- **DNS / custom domains** pointing at Pages or deployed services.")
    w(
        "- **External CI/CD targets & deploy paths** (`/opt/<repo>`, servers) keyed to repo name."
    )
    w(
        "- **Secret VALUES** (never exported by the API) — rotate from the source of truth."
    )
    w()
    ...
    w(
        "3. Transfer repos (redirects auto-created); re-add secrets/keys/webhooks/rulesets."
    )
    w("4. Repoint hardcoded refs (§2 ⚠️) via the OOTB-5 indirection.")
    w("5. Update the outside-GitHub surface (§3).")
    w(
        "6. Detach org from the enterprise (security config no longer applies → re-apply locally)."
    )
