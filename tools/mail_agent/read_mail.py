...
def bulk_kopfsaetze(
    ...
    **106,3/s** gebündelt — Faktor 10,6. Der Engpass war nie der Server, sondern
    ...
    typ, daten = imap.uid("FETCH", bereich, f"(UID BODY.PEEK[HEADER.FIELDS ({KOPFFELDER})])")
...
def uidvalidity(imap: imaplib.IMAP4_SSL, ordner: str) -> int:
    ...
    if typ != "OK" or not d:
        return 0
    m = re.search(rb"UIDVALIDITY\s+(\d+)", d[0] if isinstance(d[0], bytes) else str(d[0]).encode())
