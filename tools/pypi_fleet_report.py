...
def pin_state(ref: str | None, newest: str | None) -> str:
    """aktuell | lag | main | n/a für einen shared-ci-Pin gegen den neuesten Tag."""
    if ref is None:
        return "n/a"
    if ref == "main":
        return "main"
    if newest is None or semver_key(ref) is None:
        return "n/a"
    return "aktuell" if semver_key(ref) >= semver_key(newest) else "lag"
...
def shared_ci_pins(owner_repo: str, token: str) -> list[tuple[str, str, str]]:
    """(workflow-datei, ref, neuester Tag) je shared-ci-Aufruf im Repo."""
    ...
    for entry in listing:
        ...
        if not isinstance(text, str):
            continue
        for src_repo, _wf, ref in parse_reusable_refs(text):
            if src_repo.endswith("/shared-ci"):
                pins.append((name, ref, latest_tag(src_repo, token)))
    return pins
...
def _pin_cell(row: dict) -> str:
    ...
    parts = []
    for wf, ref, newest in row["pins"]:
        state = pin_state(ref, newest)
...
def render(rows: list[dict], generated_at: str, sources: dict[str, str]) -> str:
    ...
    counts = {
        ...
        "lag": sum(
            any(pin_state(ref, nw) == "lag" for _, ref, nw in r["pins"]) for r in rows
        ),
        "ci_red": sum(
    ...
    out = [
        ...
        *[f"- {k}: {v}" for k, v in sources.items()],
        ...
        f"| shared-ci-Pin hinter neuestem Tag | {counts['lag']} |",
