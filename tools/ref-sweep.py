...
def main() -> int:
    ...
    ap.add_argument(
        "--pin", default=None, help="repin @ref to this tag (e.g. v1.0.0); kills @main"
    )
    ap.add_argument("--owners", default="achimdehnert,iilgmbh")
    ap.add_argument(
        "--apply",
        action="store_true",
        help="open one PR per consumer (default: dry-run)",
    )
    ap.add_argument(
        "--limit",
        type=int,
        default=0,
        help="canary: only first N affected repos on --apply",
    )
    ap.add_argument(
        "--exclude",
        default="bfagent",
    ...
    if not repos:
        print(
            "::error:: no repos readable (token/scope?) — refusing to report 'clean'",
            file=sys.stderr,
        )
        return 1
    ...
    if only:
        ...
        print(
            f"# --only restricted to ({len(repos)}): {', '.join(repos) or '∅ (keine Treffer!)'}"
        )
    ...
    print(
        f"# {len(repos)} repos · owners {args.owners} · "
    ...
    if gaps:
        print(
            f"# ⚠️ {len(gaps)} unreadable file(s) — NOT 'clean': {', '.join(gaps[:5])}"
        )
    if not args.apply:
    ...
    for full in targets:
        ...
        prs, _ = gh_json(
            f"repos/{full}/pulls?head={full.split('/')[0]}:{branch}&state=open"
        )
        ...
        for f in wf if isinstance(wf, list) else []:
            ...
            _, err = gh(
                ...
                "-f",
                f"message=chore(ci): ref-sweep {old} → {new}"
                + (f" @{pin}" if pin else ""),
                "-f",
                f"content={b64}",
                "-f",
        ...
        out, err = gh(
            ...
            f"repos/{full}/pulls",
            "-f",
            f"title=chore(ci): ref-sweep {old} → {new}",
            "-f",
            f"head={branch}",
            "-f",
            f"base={db}",
            "-f",
            "body=Automated OOTB-A ref-sweep (KONZ-002): repoint shared-CI `uses:` refs."
            " See docs/runbooks/KONZ-002-ootb5-ref-sweep.md.",
        )
        print(
            f"  {'PR opened (' + str(changed) + ' files)' if out else 'PR FAILED: ' + (err or '')[:60]} — {full}"
        )
