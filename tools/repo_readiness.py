...
def read_repo_type(repo: Path) -> str:
    ...
    if (repo / "manage.py").exists():
        return "django"
    if (
        list(repo.glob("*_mcp/server.py"))
        or list(repo.glob("**/server.py"))[:1]
        and "mcp" in repo.name
    ):
        return "mcp"
    if list(repo.glob("**/klickdummy/*/screens-spec.yaml")) or (repo / "src").glob(
        "*/snippets/skins"
    ):
...
def check_git(repo: Path, fix: bool, findings: list[dict]) -> None:
    ...
    rc, counts = sh(
        ["git", "rev-list", "--left-right", "--count", "HEAD...@{upstream}"], cwd=repo
    )
    ahead = behind = 0
    ...
    if is_dirty:
        status, action = (
            "warn",
            "Working-Tree dirty — NICHT auto-gefixt (Datenverlust-Schutz).",
        )
        ...
        status, action = (
            "warn",
            f"{behind} commits behind origin (kein Fix: --report-only).",
        )
    findings.append(
        {
            "layer": "freshness",
...
def check_install_consistency(repo: Path, fix: bool, findings: list[dict]) -> None:
        ...
        findings.append(
            {
                "layer": "freshness",
                "check": "install",
                "detail": "kein pyproject [project].name",
                "status": "skip",
                "action": "",
            }
        )
    ...
    version_mismatch = (
        bool(src_ver)
        and inst_ver not in (src_ver, None)
        and inst_ver != "NOT-INSTALLED"
    )
    ...
    why = (
        ...
        if inst_ver == "NOT-INSTALLED"
            ...
            "stale editable-target → " + str(editable_path)
            if stale_target
            else f"version {inst_ver}≠{src_ver}"
        )
    )
    if fix:
        rc, out = sh(
        ...
        findings.append(
            {
                "layer": "freshness",
                "check": "install",
                "detail": detail + f" → repoint rc={rc} now={new_ver}",
                "status": "fixed" if ok else "fail",
                "action": f"pip install -e . ({why})",
            }
        )
    else:
        findings.append(
            {
                ...
                "detail": detail,
                "status": "warn",
                "action": f"editable-Repoint nötig ({why}) — mit Fix-Modus behebbar",
            }
        )
        ...
        findings.append(
            {
                "layer": "quality",
                "check": "teste_repo",
                "detail": "scripts/teste_repo.py fehlt",
                "status": "skip",
                "action": "",
            }
        )
    ...
    findings.append(
        {
            "layer": "runtime",
            "check": "import",
            "detail": f"import {mod}: {out.strip()[:80]}",
            "status": "ok" if rc == 0 else "fail",
            "action": "",
        }
    )
...
def smoke_renderer(repo: Path, findings: list[dict]) -> None:
    ...
    if any(markers.values()):
        findings.append(
            {
                "layer": "runtime",
                "check": "renderer-markers",
                "detail": ", ".join(
                    f"{k}={'✓' if v else '✗'}" for k, v in markers.items()
                ),
                "status": "ok" if all(markers.values()) else "warn",
                "action": "Voll-Smoke via Playwright im Skill-Schritt",
            }
        )
        ...
        findings.append(
            {
                "layer": "runtime",
                "check": "mcp-health",
                "detail": "ORCHESTRATOR_HEALTH_URL nicht gesetzt — übersprungen",
                "status": "skip",
                "action": "",
            }
        )
    ...
    findings.append(
        {
            "layer": "runtime",
            "check": "mcp-health",
            "detail": f"{base} → {out.strip()}",
            "status": "ok" if out.strip() == "200" else "warn",
            "action": "",
        }
    )
...
def check_runtime(repo: Path, rtype: str, findings: list[dict]) -> None:
    ...
    if rtype in ("python-package", "renderer", "klickdummy") or list(
        repo.glob("**/klickdummy")
    ):
        ...
        findings.append(
            {
                "layer": "runtime",
                "check": "django",
                "detail": "→ /frontend-ui-test bzw. /pre-release-test (Views/CSRF/DNS)",
                "status": "skip",
                "action": "delegieren",
            }
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        "--report-only", action="store_true", help="Keine Auto-Fixes (default: fixen)"
    )
