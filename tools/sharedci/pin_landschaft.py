...
gesamt = {}
for repo in REPOS:
    dateien = gh("api", f"repos/{repo}/contents/.github/workflows", "-q", ".[].name").split()
    for f in dateien:
        ...
        try:
            ...
            continue
        for wf, ref in re.findall(r"shared-ci/\.github/workflows/([\w.-]+)@([\w.\-/]+)", t):
            gesamt.setdefault(wf, {}).setdefault(ref, []).append(f"{repo.split('/')[1]}:{f}")
