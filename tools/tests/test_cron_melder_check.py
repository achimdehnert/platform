...
def _lauf(zeit, conclusion="failure", name="Melder", status="completed"):
    return {"name": name, "status": status, "conclusion": conclusion,
            "created_at": zeit, "html_url": f"https://example.invalid/{zeit}"}
...
def test_should_collect_only_scheduled_workflows(tmp_path):
    _schreibe(tmp_path, "cron.yml", "name: Melder\non:\n  schedule:\n    - cron: '0 5 * * *'\n")
    _schreibe(tmp_path, "pr.yml", "name: Nur PR\non:\n  pull_request:\n")
    assert cmc.geplante_workflows(tmp_path) == {"Melder": tmp_path / "cron.yml"}


def test_should_skip_workflows_marked_red_is_a_finding(tmp_path):
    """Ein roter Lauf ist dort ein FUND, kein Defekt — nicht als blinder Melder zählen."""
    _schreibe(tmp_path, "reconcile.yml",
              "# ROT-IST-BEFUND — absichtlich rot bei Drift\n"
              "name: Reconcile\non:\n  schedule:\n    - cron: '17 5 * * *'\n")
    assert cmc.geplante_workflows(tmp_path) == {}


def test_should_read_yaml_extension_too(tmp_path):
    _schreibe(tmp_path, "x.yaml", "name: Melder Y\non:\n  schedule:\n    - cron: '0 1 * * *'\n")
    assert "Melder Y" in cmc.geplante_workflows(tmp_path)


def test_should_strip_quotes_from_workflow_name(tmp_path):
    _schreibe(tmp_path, "q.yml", "name: \"Melder Z\"\non:\n  schedule:\n    - cron: '0 1 * * *'\n")
    assert "Melder Z" in cmc.geplante_workflows(tmp_path)
...
def test_should_stay_silent_on_a_single_red_run():
    ...
    runs = [_lauf("2026-07-28T06:00:00Z"),
...
def test_should_sort_runs_itself_and_not_trust_input_order():
    runs = [_lauf("2026-07-20T06:00:00Z", "success"),
...
def test_should_ignore_runs_still_in_progress():
    ...
    runs = [_lauf("2026-07-28T06:00:00Z", None, status="in_progress"),
            _lauf("2026-07-27T06:00:00Z"),
            _lauf("2026-07-26T06:00:00Z")]
    assert cmc.rote_serien(runs, 3) == {}
...
def test_should_report_cancelled_run_as_not_red():
    ...
    runs = [_lauf("2026-07-28T06:00:00Z", "cancelled"),
            _lauf("2026-07-27T06:00:00Z"),
            _lauf("2026-07-26T06:00:00Z")]
    assert cmc.rote_serien(runs, 3) == {}
