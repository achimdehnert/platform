...
def test_should_report_covered_when_every_probe_is_present(tmp_path):
    ...
    gate = {"slug": "lint", "drill": "drill.py", "faengt": [
...
def test_should_search_drill_extra_files_too(tmp_path):
    ...
    (tmp_path / "suite.sh").write_text("check 'T11 E402 blockt' deny\n", encoding="utf-8")
    gate = {"slug": "lint", "drill": "drill.py", "drill_extra": ["suite.sh"],
            "faengt": [{"fall": "E402", "probe": "e402"}]}
...
def test_should_report_a_luecke_when_the_drill_file_is_missing(tmp_path):
    ...
    gate = {"slug": "g", "drill": "gibtsnicht.py",
...
def test_should_stay_silent_in_kurz_mode_without_luecken():
    staende = [{"slug": "a", "zustand": "gedeckt", "gedeckt": ["x"], "fehlend": [], "dateien": 1},
               {"slug": "b", "zustand": "ungeprueft", "gedeckt": [], "fehlend": [], "dateien": 0}]
    assert gn.bericht(staende, kurz=True) == ""


def test_should_speak_up_in_kurz_mode_on_a_luecke():
    staende = [{"slug": "lint", "zustand": "luecke", "gedeckt": [], "fehlend": ["E402"], "dateien": 1}]
...
def test_should_name_unchecked_gates_in_the_full_report():
    staende = [{"slug": "b", "zustand": "ungeprueft", "gedeckt": [], "fehlend": [], "dateien": 0}]
...
def test_every_gate_with_faengt_in_the_real_registry_is_wellformed():
    ...
    reg = json.loads((Path(__file__).resolve().parents[2]
                      / "docs" / "governance" / "gate-registry.json").read_text(encoding="utf-8"))
