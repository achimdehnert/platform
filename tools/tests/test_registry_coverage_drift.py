...
def test_should_report_zero_drift_when_aligned():
    res = rcd.compute_drift(
        {"achimdehnert/a"}, _canon(("achimdehnert/a", "production", True, True))
    )
...
def test_should_classify_owner_migration_single_candidate():
    res = rcd.compute_drift(
        {"iilgmbh/a"}, _canon(("achimdehnert/a", "experimental", False, True))
    )
    assert res["migrated"] == [
        {"repo": "a", "canonical": "achimdehnert/a", "reality": "iilgmbh/a"}
    ]
...
def test_should_weight_production_phantom_as_critical():
    res = rcd.compute_drift(
        set(), _canon(("achimdehnert/ghost", "production", True, True))
    )
...
def test_should_weight_experimental_phantom_as_warn_not_critical():
    res = rcd.compute_drift(
        set(), _canon(("achimdehnert/old", "experimental", False, True))
    )
...
def test_should_flag_schema_incomplete_when_no_explicit_owner():
    res = rcd.compute_drift(
        {"achimdehnert/x"}, _canon(("achimdehnert/x", None, None, False))
    )
...
def test_should_apply_meta_repo_owner_override(tmp_path):
    ...
    assert (
        "iilgmbh/foo" in out and out["iilgmbh/foo"]["owner_explicit"] is True
    )  # Override greift
    assert (
        "achimdehnert/bar" in out and out["achimdehnert/bar"]["owner_explicit"] is False
    )  # default → schema-incomplete
