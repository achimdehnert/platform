...
def _frisch(name: str, zeilen: int = 500, fenster: list[str] | None = None) -> Hostbild:
    return Hostbild(
        name=name,
        zeilen=zeilen,
        juengste=_jetzt() - timedelta(minutes=2),
        fenster=fenster or [],
    )
...
def test_should_skip_wenn_kein_host_erreichbar_ist():
    ...
    status, text = _bewerte(
        [
            ...
            Hostbild(
                name="prod-b", fehler="kein Protokoll (Rekorder nicht installiert?)"
            ),
        ]
    )
    assert status == "SKIP"
...
def test_should_pass_aber_nicht_messbare_hosts_benennen():
    status, text = _bewerte(
        [_frisch("prod"), Hostbild(name="prod-b", fehler="ssh gescheitert")]
    )
    assert status == "PASS"
...
@pytest.mark.parametrize(
    "befund_zeile,erwartet",
    [
        (
            "2026-09-02T13:20:00Z host=p ziel=1/5 kontrolle=5/5 ip=1.2.3.4 befund=registry",
            True,
        ),
        (
            "2026-09-02T13:20:00Z host=p ziel=5/5 kontrolle=5/5 ip=1.2.3.4 befund=ok",
            False,
        ),
        (
            "2026-09-02T13:20:00Z host=p ziel=1/5 kontrolle=1/5 ip=1.2.3.4 befund=leitung",
            False,
        ),
    ],
)
