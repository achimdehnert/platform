...
def _git(
    cwd: pathlib.Path, *args: str, env: dict | None = None
...
def test_should_classify_dirty_worktree_as_skip_even_when_mergeable(
...
def test_should_skip_detached_head(tmp_path):
    ...
    verdict, reason = rw.classify(
        _wt_dict(other, None, detached=True),
        primary=str(repo),
        current=str(tmp_path / "current"),
        repo=None,
        stale_days=14,
...
def test_should_keep_when_lease_active_even_if_commit_old(tmp_path, monkeypatch):
    repo = _make_repo(tmp_path)
    wt = _add_worktree(
        repo, tmp_path, "wt-lease-active", "feature-lease-act", days_old=30
    )
    monkeypatch.setattr(rw, "pr_state", lambda branch, repo: "none")
    monkeypatch.setattr(rw, "LEASE_DIR", tmp_path / "leases")
    (tmp_path / "leases").mkdir()
...
def test_should_fallback_to_mtime_when_no_lease_present(tmp_path, monkeypatch):
    repo = _make_repo(tmp_path)
    wt = _add_worktree(
        repo, tmp_path, "wt-no-lease-old", "feature-no-lease-old", days_old=30
    )
    monkeypatch.setattr(rw, "pr_state", lambda branch, repo: "none")
    monkeypatch.setattr(
        rw, "LEASE_DIR", tmp_path / "leases-empty"
    )  # existiert nicht -> kein Lease
    verdict, reason = rw.classify(
        _wt_dict(wt, "feature-no-lease-old"),
        primary=str(repo),
        current=str(tmp_path / "current"),
        repo=None,
        stale_days=14,
...
def test_should_keep_fresh_worktree_without_lease(tmp_path, monkeypatch):
    ...
    verdict, reason = rw.classify(
        _wt_dict(wt, "feature-no-lease-fresh"),
        primary=str(repo),
        current=str(tmp_path / "current"),
        repo=None,
        stale_days=14,
...
def test_should_keep_stale_worktree_without_include_stale_flag(tmp_path, monkeypatch):
    ...
    monkeypatch.chdir(repo)
    monkeypatch.setattr(
        "sys.argv", ["worktree-reaper.py", "--apply"]
    )  # ohne --include-stale
...
def test_should_remove_stale_worktree_with_include_stale_flag(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "sys.argv",
        ["worktree-reaper.py", "--apply", "--include-stale", "--stale-days", "14"],
    )
